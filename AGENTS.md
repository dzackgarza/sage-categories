...
- **A red gate you inherited is yours, and `[known red: ...]` is not a way to keep it.** The protocol above is written for a gate you broke; it binds identically to one that was already red when you arrived. Finding a failure in place is not evidence that it is someone else's, that it is pre-existing in any sense that reduces your obligation, or that it is a papercut to route around — it means the last worker left it, which is the thing this rule exists to stop happening twice. You have exactly two honest moves: repair it, or record it as a blocker with a reproducer and an owner and then work something the gate does not touch. Annotating a commit subject and proceeding is neither.

  The recorded known-red exception under [Verification](#verification) is a **bounded** allowance for a named check with a named owner and a repair in the active plan. It is not a standing condition, and it does not survive being inherited: if you are about to write `[known red: X]` for an `X` you did not diagnose this session, X is not a known-red exception, it is an unowned defect and you are the one who found it. A check that has been red across many commits by many workers has stopped being a gate — it cannot distinguish your commit from the hundred before it — so the exception is doing the opposite of its purpose, keeping a broken check alive instead of getting it fixed.

  **Read what the check actually printed, not the line the harness wrote about it.** A QC step's summary sentence is the harness's interpretation of an exit code and it can name the wrong owner entirely. On 2026-09-11 the commit tier printed `ERROR: mypy found type errors in project code.` while mypy had never started: the step died resolving `sage-categories-homotopy`, a uv workspace member that `uvx --with` cannot see, and the harness reported the resolver's exit as a type-checking result (`ai-review-ci#409`). Anyone who trusted that sentence went looking in the source for findings that were never emitted. Before acting on any gate failure, read the output *above* the summary line and establish which tool failed and whether it produced diagnostics at all. A tool that could not run and a tool that found defects are different states, and only one of them is about your code.
