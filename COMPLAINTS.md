...
- **Current dense-source obligation:** The six-file diagnostic at `3c2023d`, explicitly including the three corrected owners and all three retained dense drafts, reports 43 errors in the dense stubs (`/tmp/sage-stubs-dense-after-inverse.log`). T05.1/T05.2 own the remaining generic contracts; T05.3/T05.5 own the dense methods. Source inspection confirms that the integer and rational stubs still duplicate inherited `row_space`, solve, and polynomial-wrapper methods. Not every conflict is a fabricated declaration: generic `matrix_from_columns` uses `PySequence_Fast` (`matrix1.pyx:2023`) while the rational override calls `len(columns)` (`matrix_rational_dense.pyx:208`); their admitted iterator domains really differ. The complete native diagnostic `/tmp/sage-stubs-native-matrix-override-domain.sage` and its `.log` reproduce three such discrepancies: an iterator works through `Matrix1.matrix_from_columns` but the rational override rejects it; `Matrix0.inverse_of_unit(..., algorithm='df')` works on an integer identity matrix but its integer override rejects the keyword; `Matrix2.echelon_form(..., cutoff=0)` works on an integer matrix but its integer override rejects `cutoff`. Each probe checks the ancestor's actual matrix result before checking the concrete call's exception. Generic and concrete `randomize` and decomposition signatures also differ by source inspection. Preserve those real argument domains and complete coefficient-changing outputs; neither deleting an inherited operation's stronger result type nor fabricating backend parameters resolves the contract. A normal installation from the retained source followed by the three-file strict check independently reproduces the same 43 errors (`/tmp/sage-stubs-inverse-install-and-dense.log`). The three dense drafts remain staged; no parent dependency pin, parent gate restoration or full matrix acceptance is claimed.
