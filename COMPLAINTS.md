...
- **Evidence:** A local Sage 10.9 / Python 3.14.7 environment was successfully provisioned at `/tmp/sage314` and is about 2.4 GiB. During 2026-09-10 public-consumer runs the root filesystem fell below 0.6 GiB free and the host used about 11 GiB of swap. Single Sage/Julia/Catlab consumers repeatedly spent minutes in `folio_wait_bit_common` before reaching test bodies. The #31 residue construction nevertheless crossed the previously failing tensor/functor placement path after `5568f78`; subsequent cold-start diagnostics were stopped to avoid exhausting the host. Unrelated resident workloads were left untouched.
