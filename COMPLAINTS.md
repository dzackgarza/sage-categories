...
- **2026-09-17 declared-runtime acceptance failure (`completion-runtime-beta10`):** Rechecked the clean repository at `60fc9581477591259ef4fe33ae64eb2de374ecf5`, using `.envrc`'s declared `SAGE_BIN=$HOME/sage-mypy-plugin/sage-stubs/sage-src/sage`, not an older installation. The fork checkout is on `develop` at `9278f565924eac778115036ec9a2eeed45509a61`. Its executable reports SageMath 10.10.beta10, and `-python` selects that checkout's `.venv/bin/python3`, CPython 3.14.3, but `import sage.all` fails with `ModuleNotFoundError: No module named 'sage'`.
  With that `SAGE_BIN` exported, `PYTHONPATH="$PWD/src" "$SAGE_BIN" -python -c 'import sage_categories'` reaches `kernel/sage_runtime.py` and fails at its first Sage import with the same exception. The exact repository acceptance command, `bash scripts/run_sage_test_case.sh tests/kernel/test_declared_python_runtime.sage test_declared_sage_python_runtime`, exits 1 in `src/bin/sage-preparse` importing `sage.misc.temporary_file`; no pytest assertion executes.
  The 14:23 UTC process inspection found no live Sage build/compiler process. The fork's `logs/install.log` and `logs/pkgs/singular-4.4.1.log` remain at 7,041,157 and 78,162 bytes respectively, both last modified at 00:25:23 UTC; their tails end during Singular's `libpolys` configuration. These observations establish an incomplete runtime and no observed continuing build, not the cause of the interruption. The fork's existing uncommitted `justfile` changes were left untouched.
  **Repair and acceptance:** Resume and finish the same external fork build/install; then rerun repository bootstrap and the unchanged declared-version consumer under the declared `SAGE_BIN`. The launcher's version string alone does not close the node. Closed source-completion nodes stay closed, and the integrated static/runtime successors remain dependent on the runtime node. This failure occurs with `SAGE_BIN` set, so it is not the recorded unset-`SAGE_BIN` source-checkpoint exception.

- **2026-09-17 completion-runtime correction:** The missing-Sage state above is superseded by the retained fork's later durable build state. At fork revision `f9bad8f987b02a95cb2325d772a7eb10e35dcea8`, `logs/runtime-sage-final-install-20260917.log` records the existing `build/meson-python` tree completing all 4,657 Meson/Ninja actions, producing `sagemath-10.10b10-cp314-cp314-linux_x86_64.whl`, and installing `sagemath==10.10b10`; the declared launcher then reports Python 3.14.3 and Sage 10.10.beta10 and imports `sage` successfully. The Sage build is complete and must not be restarted.
  Repository bootstrap now fails later at the Catlab Julia boundary. The retained shared Catlab JuliaPkg `Project.toml` has the current `PythonCall = "=0.9.35"` compatibility required by `juliacall==0.9.35`, but its preserved `Manifest.toml` still records `PythonCall 0.9.31` from the superseded 2026-09-15 accommodation; ordinary resolution rejects that stale shared manifest. With ordinary CondaPkg resolution, bootstrap instead tries to create a Pixi-managed Python under `~/.cache/sage-categories/catlab-juliapkg/.CondaPkg` and fails with `ENOSPC`. PythonCall's documented existing-Python route uses `JULIA_CONDAPKG_BACKEND=Null` and `JULIA_PYTHONCALL_EXE` pointing at the selected Python; using `Null` without the executable was the cause of the earlier misleading precompile attempt.
  Re-running with both settings, the declared Sage Python, the existing Julia 1.12.7 executable, `JULIA_NUM_PRECOMPILE_TASKS=1`, and the already-retained `/dev/shm/sage-src-meson-20260917T1854Z` staging successfully creates a current JuliaPkg project. It writes a manifest containing `PythonCall 0.9.35`, `Catlab 0.17.6`, and `GATlab 0.2.4`; `Pkg` itself completes serial precompilation in about 209 seconds and the transaction continues compiling the resolved dependency set. The exact declared-version consumer reaches pytest collection under Python 3.14.3. Root headroom nevertheless falls from about 89 MiB to 27 MiB during that continuing package precompile even with `TMPDIR` and the writable Julia depot placed in the retained staging, so the run was stopped before an uncontrolled `ENOSPC`; its residual Julia child was terminated by exact PID. The exact remaining acceptance is unchanged: attribute or relocate the remaining root write footprint, finish repository bootstrap against this one source-built Sage runtime, then execute the unchanged declared-version consumer. This evidence does not authorize a second Sage, a clean/reconfigure of the retained fork, deletion of unknown cache state, or a return to the distribution `make` route.

- **2026-09-18 completion-runtime resolution:** Fresh inspection supersedes the 27 MiB blocker state above: `/` had about 1.8 GiB free and `/dev/shm` about 1.9 GiB free before resumption, with no live Julia, Sage, Meson, or Ninja process. The root write targets were attributable rather than hypothetical: `julia_bridge._main()` defaults its JuliaPkg project to `~/.cache/sage-categories/catlab-juliapkg`, the prior run had updated that project's Project/Manifest and `~/.julia/logs/manifest_usage.toml`, and the targeted Sage runner defaults its temporary worktree to `/tmp`. The resumed run kept the mutable Catlab project, Julia compiled depot, XDG cache, and package temporary files under the existing `/dev/shm/sage-src-meson-20260917T1854Z` staging while retaining the existing root Julia depot only as a secondary package/artifact source. The old root Catlab manifest and Julia usage-log mtimes remained unchanged.
  Serial precompile then completed 17 dependencies in 579 seconds, including `PythonCall 0.9.35`, `GATlab 0.2.4`, and `Catlab 0.17.6`; a fresh Julia 1.12.7 process loaded all three with `JULIA_PYTHONCALL_EXE=/home/dzack/sage-mypy-plugin/sage-stubs/sage-src/.venv/bin/python3` and reported `CATLAB_BOOTSTRAP_OK`. Repository import/bootstrap through the declared Sage launcher reused the staged Project/Manifest unchanged and reported `REPOSITORY_BOOTSTRAP_OK 3.14.3 0.1.0`. The unchanged `scripts/run_sage_test_case.sh` consumer passed `tests/kernel/test_declared_python_runtime.sage::test_declared_sage_python_runtime`. Its first passing run used a `/dev/shm` `TMPDIR`, after which the runner returned 1 only because its `gio trash` cleanup reports “Trashing on system internal mounts is not supported”; an independent probe reproduced that cleanup behavior. Re-running the same unchanged runner with only its small `mktemp` tree under dedicated root-backed `/tmp/sage-categories-beta10-runtime` produced `1 passed` under Python 3.14.3 and an outer runner exit status of 0. Root headroom remained about 1.7 GiB after acceptance, the repository tree was otherwise clean, and no Sage or Julia runtime process remained. This closes `completion-runtime-beta10`; the earlier build, stale-manifest, Pixi, and 27 MiB observations remain historical failure evidence rather than current blockers.

- **2026-09-17 lock/group repair:** The external fork's `pyproject.toml` declared `research-environment`, but its stale `uv.lock` omitted the group. Resolution also treated the root's dynamic version as a reason to invoke meson-python's full editable build. The repair makes the release version static, maintains it through `tools/update-version`, and regenerates the lock. A version-scoped COIN 10.4 metadata entry retains its exact published `sphinx` requirement, avoiding an isolated Sage 10.7 build just to read that optional extra's legacy metadata. The final resolution completed with compilation disabled; the ordinary `uv lock --check` and exact frozen `research-environment` dry-run both pass (`logs/runtime-frozen-research-20260917.log` in the fork). All original dependency groups, extras, sources, and build settings are retained; release-edit checks preserve the version-scoped dependency metadata.
  **Remaining boundary:** External `pyproject.toml`, `tools/update-version`, and `uv.lock` remain staged at `9278f56592`: the normal hook fails with `Justfile does not contain recipe test-commit`. The inherited modified `justfile` and `build/meson-python/.ninja_log` are byte-for-byte unchanged, with 167 compiled extensions preserved. The recipe names a separate, absent `builddir`; reconcile its Meson installation route rather than start a second cold build. No dependency installation, successful bootstrap, or declared-version consumer acceptance follows from the dry-run. The current TODO's Meson-route owner correction remains authoritative; do not repeat the distribution build.
