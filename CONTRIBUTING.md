...
| `POL-CAT-085` | Replace Sage's category-only `super_categories()` edges with `structure_functors()`. Return the complete tuple of immediate `Functor` values. Selection in this tuple makes a functor structural; do not add a `StructuralFunctor` subtype or registry. Use `SubcategoryInclusion`, `FullSubcategoryInclusion`, or `WideSubcategoryInclusion` only for actual inclusions. Use category-owned `forget` functors when object maps discard structure, even when those functors are full or faithful. Use construction-owned projections for selected components. Do not infer the kind of functor from its properties, a category pair, or shared Python storage. See `specs/functor.md`. |
