...
| `POL-CAT-002` | Use `ObjectType`, `ElementType`, and `MorphismType` for category-owned implementations. The kernel owns `Cat`; every repository category uses `Cat().ObjectType`, every functor uses `Cat().MorphismType`, and every natural transformation uses `Fun.MorphismType`. `Cat()` is an object of `Cat()` by a stated runtime convention: size is outside the model, no kernel operation quantifies over or scans the objects of `Cat()`, and `Cat().category() is Cat()` is never read as an existence theorem. |
...
| `POL-CAT-061` | Compile an inherited declaration into the descendant role MRO. Execute it on the original descendant instance. Before the local constructor chain starts, compute the declaring role's constructor data through every complete selected-functor route. Never replace the receiver merely to call the method. |
...
| `POL-CAT-071` | Reject a selected structural edge when an inherited target role needs constructor data and the functor supplies no exact typed conversion. Compute conversions along structural edges before the C3 constructor chain starts. Never derive a conversion from adjacent classes in the linear order. |
...
| `POL-LEAF-047` | Give each local implementation constructor one exact typed datum containing only the new semantic data required by its category. Let selected functors convert it into the data required by ancestor role constructors. Keep the root role identity separate from every category-specific datum. |
...
| `POL-LEAF-049` | Select each immediate structural functor in the category layer. Reuse the exact functor retained by the defining category construction. Otherwise, define its maps once through `Fun(self, Target)`. Select the strongest established functor-property subcategory before construction. Construct exact images through target-category constructors. Retain the three typed constructor-data conversions required for object, element, and morphism inheritance. Add no element action beyond the action derived from morphisms. |
...
| `POL-KERNEL-020` | Compile inherited operations only. Never route a locally owned operation into Sage or another engine, replace its executable method, match it to an engine method by name, or interpret a decorator, descriptor, annotation, registry entry, or marker as a computation route. |
...
| `POL-KERNEL-028` | Keep each `DeclaredObjectType`, `DeclaredElementType`, and `DeclaredMorphismType` distinct from its compiled public role. Build the public role with Sage's dynamic-class technique. Copy the complete local namespace onto the controlled compiled-ancestor bases. Rebind each copied `__class__` closure to the compiled class. The exact MRO contains one compiled class per reachable role node, then the kernel base. |
| `POL-KERNEL-029` | Before local initialization starts, traverse the structural graph and compute one exact constructor datum for each reachable role node. Check all routes to a common node by canonical-image and datum identity. Activate a role-specific construction context. Each generated class wrapper reads its node's datum and invokes its rebound local constructor. Each local constructor initializes only its new state and calls `super().__init__()` once. C3 initializes each node once, even when adjacent C3 classes have no structural edge. The kernel base consumes the separate root identity. |
...
| `POL-FUN-035` | A selected functor retains one pure typed constructor-data conversion for each object, element, and morphism role that its codomain initializes. Each conversion maps the source node's local datum to the target node's local datum. It does not inspect a partly initialized value. Identity and composite functors retain the corresponding identity and composite conversions. |
