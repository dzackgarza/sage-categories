...
| `POL-CAT-033` | Define a subcategory only for a genuine mathematical property or structure. Additive and multiplicative refinements expose the chosen binary law through `+` or `*`. Never define a subcategory only to select, store, or expose an implementation. |
...
| `POL-CAT-069` | Give each property subcategory one same-object refinement constructor that trusts its defining property. Do not add checked, hypothesis-backed, or theorem-backed refinement families. An ambient value's owned predicate performs any check; `assume(P(x))` and exact `True` invoke the same refinement constructor; a named mathematical construction returns through it directly. This rule does not limit ordinary constructors from supported mathematical or engine representations. |
...
| `POL-CAT-084` | If `C.P()` is a property subcategory and `D` has a selected subcategory monomorphism into `C`, derive `D.P()` automatically as the corresponding narrowing of `D`. Define the property constructor once. Never require a descendant category to repeat its class, predicate, constructor, or transport wiring. |
...
| `POL-CAT-086` | Make `Mor(C)(A, B)` a category for every `A, B in C`. Define inhabitation and emptiness through the property subcategories `Cat().Inhabited()` and `Cat().Empty()`. Their containment predicates generate `is_inhabited()` and `is_empty()` on category objects. An unresolved decision leaves the morphism category symbolic. |
...
| `POL-LEAF-033` | A property-subcategory leaf declares `predicate_name`, `predicate_owner`, its containment predicate, its subcategory monomorphism, and its trusted same-object refinement constructor. The kernel derives the public application. Category placement makes `ask()` return `True`. The leaf can also declare every ordinary constructor needed for supported mathematical and engine representations. |
...
| `POL-KERNEL-026` | Propagate each property-subcategory constructor through selected subcategory monomorphisms, following Sage's `with_axiom` model. Build the descendant refinement, class MRO, constructor, and inherited predicate behavior generically. A leaf supplies no propagation machinery. |
...
| `POL-API-022` | Let each property subcategory own one trusted same-object refinement constructor for its defining property. Do not add checked, hypothesis-backed, or theorem-backed refinement families. The ambient value's predicate owns exact computation, `assume(P(x))` uses the active session, and named mathematical constructions return directly through the same refinement constructor. Ordinary representation constructors remain unrestricted by this rule. Never select evidence with a Boolean, `Decision`, default, proof object, or prose. |
