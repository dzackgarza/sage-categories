...
| `POL-SCOPE-009` | Judge an architectural claim from the live method owners, structural functors, compiled class graph, construction-input conversions, constructor obligations, and universal data. Agent reports, passing tests, route metadata, generated type identities, and runtime output cannot replace that inspection. |
...
| `POL-SCOPE-012` | Kernel code implements category-independent class compilation, structural routes, construction-input conversion, dynamic types, C3 constructor composition, and canonical images. It contains no category-specific mathematics. |
...
| `POL-MATH-046` | A structured object does not hold its underlying object; it is that object with more structure. Reject the move, whatever it is called: a stored field holding an ancestor value, a method whose first step obtains a second value standing for the one it was applied to, or an accessor named after a target category instead of after a functor. Naming is settled separately by `POL-MATH-050`, so the move is rejected even when its name reads well. For each selected `F: C -> D` the kernel makes `C.ObjectType` a Python subclass of `D.ObjectType` and threads the construction data through `D`'s initializer, so `D`'s state is the object's own. Only a named functor has an image (`POL-CAT-096`). |
| `POL-MATH-047` | A method call applies an operation. It is not a message with a recipient. Reject the move: any rule, parameter, test, or witness that distinguishes candidate values of `self`, including an assertion that a call returned the value it was applied to. `#: core(Sets()) -> Cardinal()` is a functor and `X.cardinality()` is notation for applying it. One object is present and nothing selects among candidates. |
| `POL-MATH-048` | A mathematical kind is a declaration site, never a runtime value. Reject the move: a value that names a kind, a mapping keyed by one, a branch on one, or a category assembled by passing implementation classes into a call instead of declared as a class. A category declares `ObjectType`, `ElementType`, and `MorphismType`; where the class is written is what makes it the object, element, or morphism implementation (`POL-LEAF-059`). |
...
| `POL-MATH-050` | Every technical term used in permanent documentation has a row in [specs/glossary.md](specs/glossary.md). A row is complete when it carries either an inspected external source with an exact locator, or one plain-English definition and the file and heading that states it. Writing a term with no complete row is the defect; the repair is to complete the row or to rewrite the sentence without the term. Never ask whether a term was invented. Look it up in the glossary. |
| `POL-MATH-051` | Presence of a term in this repository is not evidence that it has a meaning. Source, tests, plan text, review reports, and earlier policy rows were written by the same process under audit. Do not cite them to establish a term, and do not treat a defined-looking usage as a complete glossary row. External sources and the user establish meaning; this repository records it. |
| `POL-MATH-052` | When no external source names the thing and no plain definition fits it, the thing is an implementation artifact rather than a mathematical object. Report the missing construction. Do not open a glossary row for it. |
| `POL-MATH-053` | Prefer the plain descriptive phrase over a new compound noun. "The class compiled for objects of `C`" is admissible with no glossary row; "the object role" is a term and needs one. |
| `POL-MATH-051` | A coined term has no community that interprets it, so each reader supplies whatever meaning is convenient at that site. It is therefore more ambiguous than plain English or a mathematical statement, not less. Jargon compresses a meaning its readers already share; a coinage only imitates that compression. Admit a term when a reader outside this repository can look it up and obtain the meaning used here. A term that appears in policy, in specification, and in source while being defined nowhere has no meaning apart from the code implementing it: the policy is then the implementation described back to itself, and the code is justified by that policy. |
| `POL-MATH-052` | These coined terms are removed from permanent documentation together with the constructions they named: `carrier`, `receiver`, `role` for a Python class or for a mathematical kind, `canonical image`, and `structural_image`. Write instead the underlying object or set with its functor named; the value an operation applies to; the compiled class, or the exact mathematical type; and the image `F.on_object(x)` under a named functor. Source occurrences stay until the model that produced them is repaired (`POL-MATH-049`). |
...
| `POL-CAT-053` | A leaf never writes Python subclass relations between category-owned classes. The kernel makes each compiled `C.ObjectType`, `C.ElementType`, and `C.MorphismType` a Python subclass of the corresponding compiled classes reached through selected structural functors. A root object role continues through `ObjectOfCategory` to the one compiled `Cat().ElementType`; a root morphism role continues through `MorphismOfCategory` to that same root. An ordinary element role continues through its own element chain to `Cat().ElementType`. These generated bases are the runtime form of the functor graph and the two stated domain refinements. |
| `POL-CAT-054` | Declare every relation between categories by a selected structural functor, including a subcategory monomorphism, identity, or other trivial functor. A category without these functors is disconnected from the owned category graph. |
...
| `POL-LEAF-047` | Give each local implementation constructor one exact typed datum containing only the new state required by its category. A construction input can also carry exact semantic data consumed only by a selected functor to construct ancestor state. Keep the class identity separate from that datum. A node with no local state has no local initializer. Let selected functors convert the complete typed construction input into the target input required by ancestor class constructors. Each local initializer calls `super().__init__()` once, including the class-specific kernel initializer and the common `Cat().ElementType` initializer at the end of the chain. |
