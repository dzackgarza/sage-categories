...
| `POL-SCOPE-012` | Kernel code implements category-independent class compilation, selected-functor construction-input conversion, dynamic types, and C3 constructor composition. Each named functor owns its public images. The kernel contains no category-specific mathematics. |
...
| `POL-MATH-025` | Let `ask()` return `Unknown` only when defining data, explicit hypotheses, construction theorems, inspected sources, and available exact algorithms do not determine the applied predicate. The absence of a Python derivation does not make a theorem-established result unknown. Never convert `Unknown` to a Boolean through an unrelated proxy. |
...
| `POL-MATH-034` | Represent every mathematical truth question as an owned predicate. Applying it constructs a proposition. Represent every other mathematical query that is not total and exact on its full declared domain as an owned predicate with an exact result category. Predicate application and `ask()` are the complete public query surface. Only `ask()` returns a decision, an owned result, or `Unknown`. |
| `POL-MATH-035` | Ask every applied predicate before an assertion or branch. Write `decision = ask(proposition)`, assert that it is not `Unknown` where a decision is required, and then use the decision. Never use the applied predicate's Python truth value. |
...
| `POL-MATH-046` | A structured object is its underlying object with additional structure. For each selected `F: C -> D`, compile `C.ObjectType` as a subclass of `D.ObjectType`. Use the functor's construction-input conversion to initialize the `D` state on the same instance. Only a named functor constructs an image (`POL-CAT-096`). |
...
| `POL-MATH-048` | A category declares `ObjectType`, `ElementType`, and `MorphismType` directly as nested classes. The declaration site fixes each mathematical kind. The kernel fills the bases of those same classes. |
...
| `POL-MATH-050` | Before editing agent-facing documentation, read the forbidden-vocabulary table in [specs/glossary.md](specs/glossary.md). Replace every listed spelling and grammatical variant with its required wording. A listed term can occur only in that table and Git history. |
| `POL-MATH-051` | When a user correction or accepted review prohibits a term, add it and its exact replacement to the forbidden-vocabulary table in the same edit. Never remove a deny-list row because the current source is clean. The row prevents later reintroduction. |
| `POL-MATH-052` | Repository usage never permits a term in the deny-list. Source, tests, plans, reports, policies, and generated files cannot override the table. Only a direct user instruction can change a row. |
| `POL-MATH-053` | Use the required distinctions in [specs/glossary.md](specs/glossary.md) for points, generalized elements, functors, implementation classes, images, and constructor conversions. Do not infer an alternative term from nearby source. |
...
| ID | Policy |
| --- | --- |
| `POL-ASSUME-005` | Return a `bool` only for an implementation fact that is two-valued by construction: established placement, a record of retained data, or the presence of a chosen enumeration. Every mathematical truth question returns an applied proposition. Every other mathematical query that is not total and exact on its full declared domain returns an applied predicate with an exact result category. Only `ask()` returns `True`, `False`, an owned result, or `Unknown`. |
...
| `POL-ASSUME-012` | Never write `is True` or `is False`. Applied to a proposition they compare the object itself and never match, so the branch is dead and the failure surfaces later with a message that blames something else; applied to a decision they say nothing that using the decision does not. Branch on `ask(P)` directly. `bool(Unknown)` raises, so an undecided answer stops there and announces itself. |
| `POL-ASSUME-013` | Compose several answers by asking each: `all(map(ask, propositions))`, `any(map(ask, propositions))`. An unasked proposition folded through `all`, `any`, `and`, `or`, or `not` raises rather than deciding. Python container and tuple equality is the same trap one level down, since it takes the truth value of each component, and equal owned values survive it only by an identity fast path. |
| `POL-ASSUME-014` | Where the code requires a decided answer, write `decision = ask(proposition)` and then `assert decision is not Unknown`, with the message naming what could not be decided. The assertion is the documentation: it states that this path has no answer for an undecided input, and it fails at the place that made the assumption rather than three frames later. |
| `POL-ASSUME-015` | Use no Boolean for a mathematical question anywhere in the package. A `bool` is honest only for an implementation fact that is two-valued by construction: established placement, a record of retained data, or the presence of a chosen enumeration. Every mathematical truth question is a proposition. Every other non-total mathematical query is an applied predicate with an exact result category. `ask()` is the only public evaluation operation. |
| `POL-ASSUME-016` | Distinguish a handler that answers `False` because the proposition is false from one that answers `False` because its argument is outside the handler's domain. The second is a fabrication. A total classifier over an `Any` position may answer `False` for a value outside its accepted type (`POL-TYPE-004`); a rule that consults an engine answers `True` where the engine establishes membership and `Unknown` otherwise. |
| `POL-ASSUME-017` | Validate a constructor argument before normalizing it. Truncating, rounding, or coercing an argument first turns an ill-formed input into a well-formed wrong value, and a decision recorded about that value is cached and refines placement, so the error becomes permanent and invisible. |
...
| `POL-ASSUME-007` | `assume(P(x))` supplies the hypothesis and immediately uses the kernel's same-object refinement for the property subcategory defined by `P`. It does not run the decision procedure. |
...
The public `assume(order_preserving(f))` operation then refines the owned morphism into
its order-preserving property category.
A registered handler can instead establish the proposition for supported semantic morphism types.
When that handler returns exact `True`, it uses the same kernel refinement.
...
| `POL-CAT-012` | During source construction, compare the target constructor data supplied by all selected paths to one target class. A mismatch raises a construction-defect error naming the source construction, both paths, and the target class. Each named functor keeps its own public images. Natural transformations never identify constructor data or public images. |
...
| `POL-CAT-043` | Let a named property subcategory and its owned predicate state one mathematical condition, declared once as the subcategory's membership proposition. The predicate applies that proposition and requests a decision and self-refinement; containment asks the same proposition and forces a Boolean. They are one question asked twice, not two notions of membership. Never replace either with an unrelated invariant comparison. |
| `POL-CAT-044` | Put each exact decision procedure behind the owning mathematical predicate. The property subcategory's membership proposition is that one declaration, and the kernel-derived `is_X()` application returns it: `X.is_finite()` is the proposition `X in Sets().Finite()`, whose defining predicate `cardinality() < aleph_0` is supplied by the class implementing `Sets().Finite()`. `ask()` evaluates that proposition and `__contains__` asks the same one. Exact `True` invokes the kernel's same-object refinement mechanism, after which placement answers positively without recomputation. Do not override the predicate with a Boolean method, register the defining mathematics as a separate handler beside a placement-only membership proposition, or apply a category's predicate object at a use site. |
...
| `POL-CAT-052` | Make generic category constructors propagate through selected functors. A descendant category supplies no boilerplate merely to form the inherited construction subcategory. |
| `POL-CAT-053` | A leaf never writes Python subclass relations between category-owned classes. The kernel makes `C.ObjectType` inherit `Cat().ElementType` and the target object classes reached through selected functors. It makes `C.MorphismType = Mor(C).ObjectType`. It adds a target `ElementType` base only when the selected structural declaration supplies an exact element-constructor conversion. |
...
| `POL-CAT-055` | Treat a failed selected functor or method compiler as a foundational defect. Its failure does not permit explicit subclassing or another inheritance path. |
| `POL-CAT-056` | Apply selected-functor inheritance to every functorial, universal, and morphism-based construction. Each construction creates and retains its distinct monomorphism, projection, or evaluation functors through `Fun(Source, Target)`. The endpoint pair never selects one functor. |
| `POL-CAT-057` | Give every category its own `ObjectType`, `ElementType`, and `MorphismType` declarations. A full property subcategory self-refines the same owned value and places its category-owned class first in the Sage dynamic MRO. |
| `POL-CAT-058` | Compile `ElementType` inheritance from the point conversion of selected functors. A subcategory declares its own `ElementType`; the kernel fills its selected target bases. |
| `POL-CAT-059` | Let each category add local methods to its `ElementType`. Inherit a target element interface only through an explicit selected element-constructor conversion. Preserve the category-specific element type even when it adds no local methods. |
| `POL-CAT-060` | Every property subcategory declares `predicate_name` and `predicate_owner`, naming one public predicate and its largest meaningful ambient implementation class. The kernel derives the corresponding application there. It returns the category-owned proposition. `ask()` can use its computational routes and self-refine the ambient value. Category placement makes `ask()` return `True`; the application keeps the same proposition-valued contract. |
| `POL-CAT-061` | Compile an inherited declaration into the descendant class MRO. Execute it on the original descendant instance. Before the local constructor chain starts, compute the declaring class's construction input through every complete selected-functor path. Initialize `Cat().ElementType` on each `C.ObjectType` with parent `C`. A morphism receives this object rule through `Mor(C).ObjectType`. |
| `POL-CAT-062` | Make inherited calls use ordinary Python inheritance. A declaring category's method applies to the value it was called on and reads its initialized state there. The selected functor's conversion supplied that state (`POL-KERNEL-018`). A leaf that changes the mathematics overrides the method or adds its own. |
...
| `POL-CAT-066` | Each named functor owns its image cache. Key object and morphism images by the exact source value and mathematical kind. Two functors with the same endpoints keep independent caches. A point image uses the functor's morphism action and its declared terminal-object comparison. |
...
| `POL-CAT-069` | Use the kernel's same-object refinement mechanism for every established positive property. Do not add checked, hypothesis-backed, or theorem-backed refinement APIs. An ambient value's owned predicate performs any check; `assume(P(x))`, exact `True`, and named mathematical constructions establish the same category placement. A property category can provide every constructor required by its supported mathematical and engine representations. |
...
| `POL-CAT-071` | Reject a selected structural edge when an inherited target class needs construction input and the declaration supplies no exact typed conversion. A conversion comes from the declaration that states it, and never from whichever classes happen to sit next to each other in the compiled order. Initialize `Cat().ElementType` from the parent category of each object. When the conversions run is an implementation choice (`POL-DOC-017`). |
...
| `POL-CAT-076` | Keep mathematical type, Python call shape, and structural construction provenance distinct. Exact types state exact mathematical types. The Python signature states positional, keyword, and variadic shape. Canonical-image and construction-input relations state structural provenance. No one of these facts can replace another. |
| `POL-CAT-077` | Determine method ownership from its definition on the category-owned implementation class and the selected functors. No decorator, marker, annotation payload, registry entry, or descriptor argument can create mathematical ownership or repair a missing category declaration. |
...
| `POL-CAT-079` | Place every operation forced by category placement at the highest category that first guarantees it. A universal construction owns its shape, index, diagram, cone or cocone, defining morphisms, universal morphism, and presentation operations. The isomorphism category owns inversion; a product owns `product_projection(i)` and its universal map. Descendants receive these operations through inheritance and state only their added mathematical structure and algorithms. |
| `POL-CAT-080` | Before placing code in a leaf, trace the complete public call from its mathematical owner through construction families, selected functors, compiled class inheritance, construction-input conversion, direct inherited execution, and the declared result. Perform this trace for objects, points, and morphisms. A missing step is a foundational defect. |
...
| `POL-CAT-082` | Permit a category-owned predicate handler to return `True` when the construction or definition establishes that property for every value it builds. The public predicate still returns an applied proposition. The exact handler result invokes the kernel's same-object refinement mechanism; it is not a proof object, metadata field, or separate admission route. |
| `POL-CAT-083` | Represent a distinguished named mathematical object by its parameterized one-object category. Let a selected monomorphism place its sole object in each established ambient or property category. Install the target `ObjectType` state on that same object. If the distinguished object is a category `C`, its actual elements are the objects of `C`, so the target `ElementType` applies to `C.ObjectType`. This placement does not apply the target element class to morphisms of `C`. |
| `POL-CAT-084` | If `C.P()` is a property subcategory and `D` has a selected subcategory monomorphism into `C`, derive `D.P()` automatically as the corresponding narrowing of `D`. Derive its class, predicate, refinement, and transport wiring generically. A descendant can add constructors for its own supported representations. |
...
| `POL-CAT-086` | Make `Mor(C)(A, B)` a category for every `A, B in C`. Define inhabitation and emptiness through the property subcategories `Cat().Inhabited()` and `Cat().Empty()`. Their containment predicates generate `is_inhabited()` and `is_empty()` on category objects. An unresolved decision leaves the morphism category symbolic; it never replaces that category with an empty category. |
...
| `POL-CAT-090` | Define `Mor(Cat()).Full()`, `.Faithful()`, `.FullyFaithful()`, `.EssentiallySurjective()`, and `.Equivalences()` through the ordinary property-subcategory mechanism. The kernel derives their `is_*()` applications from the owned containment predicates. Direct construction, assumptions, category placement, and declared subcategory containments use the standard same-object refinement path. |
...
| `POL-CAT-094` | A subobject of `P` is an object `S` with a monomorphism `j: S -> P`; there is no second notion and no separate name for a representative. The equivalence-class formulation matters only when deciding whether two subobjects are equal, which is a predicate. Let `P` be a product category and let `j: S -> P` be such a monomorphism. Make the corresponding object of `Cat().Subobjects(P)` retain `j`. Define its `product_projection(i)` as `P.product_projection(i) after j`. Thus every such object has all component functors. Do not ask a leaf to repeat their maps. |
...
| `POL-CAT-096` | A functor has an image; a category does not. The image of `x` is `F.on_object(x)` for a named functor `F`, and the image of `f` is `F.on_morphism(f)`. Two functors into one target can have different images and independent caches. Selection for inheritance changes none of this. The compiler compares only the constructor data that selected paths supply to one target class. |
| `POL-CAT-098` | Objects carrying a choice of extra data form the total category of a fibration over the base, not a subcategory of it. `Modules(R)` and the category of pairs `(M, S)` with `S` a generating set are different categories, related by the functor `p: E -> Modules(R)` that forgets the datum, whose fibre over `M` is the category of choices for `M`. State that functor. The datum is normally a morphism: a generating set is an epimorphism `Free_R(S) ->> M`, a finite presentation is a length-two resolution `Free_R(X_1) -> Free_R(X_0) -> M`, a resolution of any length is the general case. So the whole family is one construction at different shapes, built from `C.SliceOver(M)`, `Fun([1], C)`, and `Fun(I, C)` (`POL-CAT-092`, `POL-CAT-095`), never a separate axiom per datum. The construction is the Grothendieck construction, an equivalence of 2-categories between pseudofunctors `C^op -> Cat` and fibrations over `C` ([nLab](https://ncatlab.org/nlab/show/Grothendieck+construction)); the fibration condition is [Stacks, Definition 4.33.5, tag 02XJ](https://stacks.math.columbia.edu/tag/02XJ). State also which morphisms the total category has: whether they must respect the datum, and whether they are cartesian over the base. Sage's `WithBasis` names the phenomenon and settles none of this - its morphisms are ordinary module morphisms while its homset reads a matrix in the distinguished bases (`sage/categories/modules_with_basis.py:47`, `:179`) - so its spelling may be borrowed only after the fibration, the fibre, and the morphisms are written down. Never `Chosen<Anything>`. |
...
- Limits, colimits, products, coproducts, tensor products, and direct sums select their named functors to the categories of their resulting objects.
...
- `Sets().Finite()` declares its monomorphism into `Sets()` even when both categories use the same realization.
  The monomorphism states the categorical relation that replaces a Sage `super_categories` declaration.
...
| ID | Policy |
| --- | --- |
| `POL-LEAF-058` | A functor transports constructor data. Declaring a supercategory does not say how to construct its objects or morphisms from the source. The leaf supplies one pure conversion from its construction data to the target constructor data. The named functor uses that conversion for its public action. The kernel uses the same conversion to initialize the target class on a structured source instance. |
| `POL-LEAF-060` | Restricting structure to a subobject is leaf work, and the compiler never learns it. For a poset presented as `(X, R)`, `Sets().Subobjects(X).from_predicate(predicate)` constructs only the selected set and its monomorphism into `X`. `PartiallyOrderedSets().Subobjects(P)` specializes the inherited subobject construction by retaining the restricted order and the defining monomorphism in `PartiallyOrderedSets()`. The ordering subtree decides whether a result lies in `PartiallyOrderedSets()` or `TotallyOrderedSets()`. The categorical theorem that transports a monomorphism between the two categories belongs to the leaf that states it. A general category never contains a specific leaf's restriction rule. |
| `POL-LEAF-059` | Declare every leaf category as a class with `ObjectType`, `ElementType`, and `MorphismType` nested directly in it. The kernel compiles those same classes. Declare each property axiom once and let Sage's axiom mechanism generate the abstract property subcategory. A separate concrete implementation category declares its own nested classes and connects itself to that axiom subcategory with `_base_category_class_and_axiom`. Each generated `D.P()` refinement uses that one axiom declaration (`POL-CAT-084`). |
...
| `POL-LEAF-003` | Explicitly construct and select each subcategory monomorphism, projection, restriction, lift, or evaluation functor. Obtain it from the defining category construction or construct it through `Fun(self, Target)`. Define its object and morphism maps when its mathematical presentation requires them. |
...
| `POL-LEAF-005` | Let the category compiler inherit the target categories' object, element, and morphism methods along selected functors. A leaf category defines no forwarding methods. |
| `POL-LEAF-006` | Treat a leaf implementation of an inherited operation as evidence of a missing selected functor, an incorrect functor image, or an operation placed at the wrong owner. |
| `POL-LEAF-007` | Permit a selected functor to land in a morphism category `Mor(C)` when the defining morphism determines the required inherited object data through its domain or codomain. |
...
| `POL-LEAF-011` | Lift an inherited construction to a leaf category by specifying only how the leaf's additional structure acts on the inherited result and morphisms. Make this lift compatible with the selected functors. |
...
| `POL-LEAF-014` | Ship and maintain a standard template for new leaf categories. The template contains only the category declaration, minimal constructor, selected functors, and sites for new methods. A template is design pseudocode that shows the required simplicity, so it is never imported, executed, type-checked, or held to a spelling criterion. It shows one way to write a leaf, chosen as a worked example. It does not fix the presentation of the mathematics it illustrates: many models of a poset are correct, and a template showing one of them does not make the others stale. A template is a defect only when it teaches a shape the kernel cannot compile. |
...
| `POL-LEAF-023` | Do not copy inherited storage, caches, or constructor arguments into a property refinement. A finite poset adds finite-poset operations and declares its subcategory monomorphisms. Same-object refinement preserves its set image, relation, elements, private realizations, and the images owned by existing named functors. The refined dynamic MRO supplies inherited operations; the leaf never traverses a monomorphism to recover its own state. |
| `POL-LEAF-024` | A finished leaf contains its category, minimal defining data, new operations, immediate selected functors, and named constructors. It can also state leaf-specific lifts. |
...
| `POL-LEAF-032` | Treat selected functors as the complete inheritance program. A finished leaf contains no forwarding, descriptor, route, cache, wrapper, or type-repair boilerplate. |
| `POL-LEAF-033` | A property-subcategory leaf declares `predicate_name`, `predicate_owner`, its containment predicate, its subcategory monomorphism, and every constructor needed for its supported mathematical and engine representations. The kernel derives the public application and performs same-object refinement. Category placement makes `ask()` return `True`. |
...
| `POL-LEAF-037` | Never discover, inspect, compose, or traverse structural routes in a leaf. Select immediate named functors and use the resulting public inherited surface. |
...
| `POL-LEAF-047` | Give each local implementation constructor one exact typed datum containing only the new state required by its category. A selected functor converts complete source construction data into the target input required by an ancestor constructor. Each local initializer calls `super().__init__()` once. An object initializer reaches `Cat().ElementType` with its parent category. |
...
| `POL-LEAF-049` | Select each immediate named functor in the category layer. Reuse the exact functor retained by the defining category construction. Otherwise, define its object and morphism maps once through `Fun(self, Target)`. Select the strongest established functor-property subcategory before construction. Supply a terminal-object comparison when the selected functor transports points. |
...
| `POL-LEAF-054` | Keep kernel concerns out of every leaf import, decorator, annotation, signature, class attribute, and method body. Leaves state mathematical data, operations, named functors, constructors, and lifts. |
...
| ID | Policy |
| --- | --- |
| `POL-KERNEL-001` | The kernel owns category-independent class compilation and selected-functor constructor composition for objects, elements, and morphisms. This includes path composition, typed construction inputs, and C3 initializer composition. Each named functor owns its public images and image cache. |
| `POL-KERNEL-002` | The kernel owns generic same-object property refinement. It strengthens the owned value's category, rebuilds its Sage dynamic MRO from category-owned class declarations, and preserves identity, construction data, and private realizations. Named functor images remain separate values. |
| `POL-KERNEL-003` | A leaf functor states its mathematical action on objects and morphisms. Its action on a point derives from its morphism action and a declared terminal-object comparison. A generalized element maps through the morphism action but remains separate from `ElementType`. The leaf never implements path composition or cache management. |
| `POL-KERNEL-004` | The kernel inherits universal constructions and their universal morphisms; it never invents mathematics for a descendant. The leaf writer supplies the mathematical delta and performs the lift: `Sets()` constructs the product of the underlying sets, and the module category states the module structure on that product. An ancestor cannot lift its own result, because nothing at that ancestor knows the descendant's structure. |
...
| `POL-KERNEL-010` | Keep dynamic-class construction and C3 initializer composition inside the kernel. Expose the resulting typed mathematical surface through ordinary Python inheritance. |
...
| `POL-KERNEL-017` | The kernel alone composes selected-functor paths and computes the construction input for each reachable class. Theory code never traverses those paths. Each named functor constructs and retains its own public images. |
...
| `POL-KERNEL-020` | Copy locally declared operations into compiled classes and inherit them through the class MRO. Never route a locally owned operation into Sage or another engine, replace its executable method, match it to an engine method by name, or interpret a decorator, annotation, registry entry, or marker as a computation route. |
...
| `POL-KERNEL-026` | Propagate each property subcategory through selected subcategory monomorphisms, following Sage's `with_axiom` model. Build the descendant refinement, class MRO, construction dispatcher, and inherited predicate behavior generically. A leaf supplies no propagation machinery and can add constructors for its own representations. |
| `POL-KERNEL-027` | Let `Fun(Source, Target)` construct functors from complete object and morphism actions. Implement identity, composition, and the monomorphisms established by subcategory data there. Let each product, pullback, comma, `Fun([1], C)`, or other category construction create and retain its own named functors. The kernel never interprets a leaf presentation to select one. |
| `POL-KERNEL-028` | An object of `C` is an instance of the compiled class of every category `C` declares a functor to, and carries the private data those inherited methods need. Methods alone do not satisfy this row. The compiled class for each mathematical kind carries the name the category wrote, `ObjectType`, `ElementType`, or `MorphismType`. How the kernel builds those classes is an implementation choice and is not fixed here (`POL-DOC-017`). |
| `POL-KERNEL-029` | A leaf constructor accepts its own category's data and initializes its own state only. It never accumulates a field for an ancestor category, and never calls an ancestor constructor by hand. Each selected functor states how the leaf's data supplies what its target's constructor requires, and the kernel uses that statement to complete construction so that every reachable category is initialized exactly once. Ordering, threading, and allocation are implementation choices and are not fixed here (`POL-DOC-017`). |
...
Selected functors are executable inheritance declarations.
...
A lattice-specific cardinality implementation signals a missing or incorrect selected functor.
...
An enumeration is additional mathematical structure and therefore belongs to a separate category with its own selected functor to `TotallyOrderedSets()`.

## Mathematical encapsulation and repository layout

| ID | Policy |
| --- | --- |
| `POL-LAYOUT-001` | Keep a leaf subtree expressed in the language of its own category, its defining structure, and its immediate selected functors. Deeply underlying operations belong to the category that owns them. |
| `POL-LAYOUT-002` | Treat a reference to cardinality inside a lattice subtree as an ownership defect. Cardinality reaches lattice objects through their selected functors to modules and sets. |
...
| `POL-FUN-002` | A functor owns its object and morphism actions. For a point `t: 1_C -> X`, its point image is the composite of the supplied morphism `c_F: 1_D -> F(1_C)` with `F.on_morphism(t): F(1_C) -> F(X)`. For a generalized element `t: T -> X`, the morphism action gives `F(t): F(T) -> F(X)`; this is not an `ElementType` value unless its domain is the terminal object. `Cat().ElementType` covers only the point case `* -> C`. |
...
| `POL-FUN-028` | Do not define a generic “forgetful functor.” That phrase does not select an object of `Fun(C, D)`. Outside the case of a right adjoint to a named free functor, it has no general definition, and a source, target, object presentation, or collection of Python fields can admit several distinct functors. Name the functor for what it constructs: `Posets() -> Sets()` is the projection `(X, R) |-> X`, a fibration. |
...
| `POL-FUN-035` | A selected functor retains one pure typed conversion from source construction data to the data consumed by each target constructor. The functor uses the conversion for its public object and morphism actions. The kernel uses the same conversion to initialize the target class on the source instance. Public `F(x)` returns the separate image owned and cached by `F`; inherited methods use direct Python MRO on `x`. Point construction uses `F.on_morphism` and the declared terminal-object comparison. Two selected paths to one target class must produce the same target constructor datum. A conversion reads no partly initialized value. |
...
retained identity-on-values monomorphism, and it retains the diagram and universal data of
...
| `POL-SET-010` | Support finite, infinite, and symbolic cardinal values. `X.cardinality()` returns an applied predicate whose result category is `Cardinal()`. `ask(X.cardinality())` returns an owned cardinal or Sage `Unknown`. A cardinal is always exact and contains no unresolved value. |
...
| `POL-API-025` | A category writes `ObjectType`, `ElementType`, and `MorphismType` directly, and the kernel exposes the compiled class under that same name, so the public API and the declaration use one name for each mathematical kind. The compiled `C.ObjectType` inherits `C`'s local declaration and inherits `F(C).ObjectType` for each selected functor `F`. Python identity with the nested declaration is not required: whether the kernel builds a new dynamic class or fills the declaration's bases is an implementation choice (`POL-KERNEL-028`, `POL-DOC-017`), and one declaration serving a parameterized family of categories is why a new class is often the right one. A leaf never builds this inheritance. |
...
| `POL-API-020` | When a mathematical operation is not total and exact on its full declared domain, return an applied predicate with the exact category of its evaluated result. Let `ask()` return an owned result or Sage `Unknown`. A truth-valued operation returns an applied proposition, and `ask()` returns `True`, `False`, or `Unknown`. Never put `Unknown` inside an owned mathematical result type. |
...
| `POL-API-022` | Use the kernel's same-object refinement mechanism for every established positive property. Do not add checked, hypothesis-backed, or theorem-backed refinement APIs. The ambient value's predicate owns exact computation, `assume(P(x))` uses the active session, and named mathematical constructions establish the property directly. A property category can provide every constructor required by its supported representations. Never select evidence with a Boolean, `Decision`, default, proof object, or prose. |
...
It returns an applied predicate with result category `Cardinal()`.
