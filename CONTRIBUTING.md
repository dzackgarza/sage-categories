...
| `POL-CAT-094` | Let `P_D` be a chosen product presentation with apex `A_D`, and let `j: S -> A_D` present a subobject. Make the corresponding object of `Cat().Products().Subobjects()` retain both `P_D` and `j`. Define its `product_projection(i)` as `P_D.product_projection(i) after j`. Thus every such object has all component functors. Do not ask a leaf to repeat their maps. |
...
| `POL-FUN-019` | Define `C.Products()`, `C.Coproducts()`, `C.TensorProducts()`, and analogous named constructions as categories of chosen presentations. For each input diagram `D`, their constructor returns a presentation `P_D` that retains its canonical apex `A_D`, defining morphisms, and universal maps. Distinct diagrams return distinct presentations, including when their canonical apexes are identical. Their selected apex functor sends `P_D` to `A_D` in `C`. Keep these categories distinct from the full replete subcategory `C.ImagesOfFunctor(F)`, which records only the existential image property. |
