...
| `POL-CAT-012` | Detect a structural-image mismatch at the first transport of a value, not during method compilation. Traverse every route to a reachable category in `structure_functors()` declaration order, cache the first image, and require each later image to be the same object by identity. A mismatch raises a construction-defect error naming the source construction, both routes, and the shared ancestor category; nothing is repaired or replaced. Natural transformations are trusted constructions and never compiler proofs; they never identify or rewrite routes. |
...
| `POL-CAT-053` | A leaf never writes explicit Python subclassing between category-owned object, element, or morphism implementations without prior user discussion and approval; hand-written subclassing bypasses the structural-functor framework. The kernel makes each compiled `C.ObjectType`, `C.ElementType`, and `C.MorphismType` a subclass of the compiled roles reached through selected structural functors, ending on the kernel role class of the role. Those generated bases are the runtime form of the functor graph. A local declaration is not among them, except where it is itself that kernel role class, which is inherited rather than copied so that its Python subclasses can still override it. |
...
| `POL-CAT-062` | Transport the receiver and every mathematical argument, including keyword arguments, forward through the same complete route. An inherited method returns the declaring method's value: `X.f() := F(X).f()`. Nothing is transported back; every result stays in its exact declared mathematical category. A leaf that wants a source-category result overrides the inherited method or adds its own. |
| `POL-CAT-063` | Preserve object, element, morphism, iterator, and mathematical collection roles in compiled method signatures. Derive these roles from the owning implementation role and exact declared types. Do not infer them from runtime registries, `isinstance`, method names, or duplicate per-method metadata. |
...
| `POL-CAT-065` | Return a lazy result and an owned collection exactly as the declaring method returned them, one value at a time. Each item is a value of the declaring category. |
...
| `POL-CAT-071` | Reject a compiled method when a declared argument requires structural transport and no exact rule exists. Never use raw pass-through of an argument as a fallback. An inherited method returns the declaring method's value in its declared category. |
...
| `POL-CAT-076` | Keep mathematical type, Python call shape, and structural transport provenance distinct. Exact types state mathematical roles. The Python signature states positional, keyword, and variadic shape. Canonical image relations state transport provenance. No one of these facts can replace another. |
...
| ID | Policy |
| --- | --- |
| `POL-KERNEL-001` | The kernel owns complete structural transport for objects, elements, and morphisms. This includes route composition, arguments, results, and canonical caches. |
...
| `POL-KERNEL-022` | Use mathematical roles to type transport and canonical image relations to decide whether transport applies. Never relabel a category, object, element, morphism, or mathematical collection as a plain value to suppress transport. |
...
| `POL-TEST-006` | Test object, element, and morphism inheritance by calling inherited public operations through the real category compiler. Route records, descriptors, generated type identities, and caches do not prove that public surface. |
