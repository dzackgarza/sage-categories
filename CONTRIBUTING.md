...
| `POL-SCOPE-009` | Judge an architectural claim from the live method owners, structural functors, compiled role MROs, constructor obligations, public functor images, and universal data. Agent reports, passing tests, route metadata, generated type identities, and runtime output cannot replace that inspection. |
...
| `POL-SCOPE-012` | Kernel code implements category-independent role compilation, constructor composition, structural routes, dynamic types, public functor application, and canonical images. It contains no category-specific mathematics. |
...
| `POL-CAT-002` | Use `ObjectType`, `ElementType`, and `MorphismType` for compiled category-owned implementations. The kernel owns `Cat`; every repository category uses `Cat().ObjectType`, every functor uses `Cat().MorphismType`, and every natural transformation uses `Fun.MorphismType`. Bootstrap uses a distinct local `CategoryDeclaration`, then binds the public `Category` name to `Cat().ObjectType`. `Cat()` is an object of `Cat()` by runtime convention: size is outside the model, no kernel operation quantifies over or scans the objects of `Cat()`, and `Cat().category() is Cat()` is never read as an existence theorem. |
...
| `POL-CAT-006` | A theory category never redeclares a method owned by another category. The kernel can copy an owning local declaration into one compiled role. It must preserve the declaration and bind it to the compiled class. |
| `POL-CAT-007` | Do not maintain an independent Python class graph. Generate the runtime role graph only from selected structural functors. |
| `POL-CAT-008` | Compile complete role declarations into public implementation classes. Generate no method body except role-constructor wrappers. |
...
| `POL-CAT-012` | Detect a structural-image mismatch during construction or the first public functor application. Traverse every route to a reachable category in `structure_functors()` declaration order, cache the first image, and require each later image to be the same object by identity. A mismatch raises a construction-defect error naming the source construction, both routes, and the shared ancestor category; nothing is repaired or replaced. Natural transformations are trusted constructions and never compiler proofs; they never identify or rewrite routes. |
...
| `POL-CAT-053` | A leaf never writes Python subclass relations between category-owned roles. The kernel makes each compiled `C.ObjectType`, `C.ElementType`, and `C.MorphismType` a Python subclass of the corresponding compiled roles reached through selected structural functors. Those generated bases are the runtime form of the functor graph. |
...
| `POL-CAT-057` | Give every category separate local object, element, and morphism implementation declarations. Compile them into the public `C.ObjectType`, `C.ElementType`, and `C.MorphismType`. A full property subcategory self-refines the same owned value and places its compiled role first in the Sage dynamic MRO. Do not allocate a wrapper or second owned value for that refinement. |
...
| `POL-CAT-065` | Return a lazy result and an owned collection exactly as the inherited method returns them, one value at a time. Do not wrap, transport, or relabel the collection or its items. |
...
| `POL-CAT-075` | Treat the ordinary typed signature and executable body on the owning local implementation class as the sole authoritative declaration of a method. Copy that declaration into the compiled role and derive generated typing artifacts from it. Never maintain a second description of its receiver, parameters, call shape, result, or mathematical roles. |
| `POL-CAT-076` | Keep mathematical type, Python call shape, and structural-constructor provenance distinct. Exact types state mathematical roles. The Python signature states positional, keyword, and variadic shape. A selected functor states how source construction supplies target implementation state. No one of these facts can replace another. |
...
| ID | Policy |
| --- | --- |
| `POL-LEAF-001` | Integrate a new leaf category by supplying its selected structural functors to known categories. These functors, including their constructor conversions, are the complete inheritance declaration. |
...
| `POL-LEAF-005` | Let the category compiler inherit the target categories' complete object, element, and morphism implementations along selected structural functors. This includes executable methods and initialized private state. A leaf category defines no forwarding methods. |
| `POL-LEAF-006` | Treat a leaf reimplementation of an unchanged inherited operation as evidence of a missing structural functor, incorrect functor image, or wrong owner. When the new structure changes the result category, define the source-category construction as an ordinary override under `POL-LEAF-017`. |
...
| `POL-LEAF-008` | Confine private-field access to local constructors and constructor conversions on genuinely new functors that cannot recover their required defining data through owned semantic interfaces. A standard kernel functor never requires leaf access code. |
...
| `POL-LEAF-023` | Do not repeat inherited storage, caches, or constructor arguments in a property refinement. A finite poset adds finite-poset operations and declares its categorical inclusions. Same-object refinement preserves all state already initialized by its compiled role constructors. The refined dynamic MRO supplies inherited operations; the leaf never traverses an inclusion to recover its own state. |
...
| `POL-LEAF-032` | Treat selected structural functors as the complete inheritance program. A finished leaf contains only its local role implementations, minimal constructor, selected functors, and the constructor conversions those functors require. It contains no forwarding, descriptor, route, cache, wrapper, or type-repair boilerplate. |
...
| `POL-LEAF-038` | Never call private image transport from a leaf for an object, element, or morphism. All such transport belongs to the kernel. |
| `POL-LEAF-039` | Call an inherited operation directly on the original structured value. The ancestor declaration executes on that value through its compiled MRO. If that call fails, stop the leaf edit and repair role construction or compilation. |
...
| `POL-LEAF-041` | Make the compiled `C.ObjectType`, `C.ElementType`, and `C.MorphismType` the sole executable public implementation classes for `C`. Each class contains the local declaration and every selected ancestor implementation, including their initialized state. It is the public firewall for that mathematical role. |
| `POL-LEAF-042` | Let the category declaration define or link exactly one local implementation declaration for each mathematical role. The kernel gives its compiled public role a different runtime class and name. After category construction, bind the module's semantic object, element, and morphism names to those compiled classes. Use those semantic names in source annotations and generated stubs. Never offer competing implementation classes, backend choices, realization variants, or parallel public surfaces for one mathematical notion. |
...
| `POL-LEAF-049` | Select each immediate structural functor in the category layer. Reuse the exact functor retained by the defining category construction. Otherwise, define its maps once through `Fun(self, Target)`. Select the strongest established functor-property subcategory before construction. Construct exact images through target-category constructors. Retain the three typed construction-input conversions required for object, element, and morphism inheritance. Add no element action beyond the action derived from morphisms. |
...
| `POL-LEAF-051` | Write a leaf method as one ordinary typed Python method. Never attach or place beside it transport metadata, compiler annotations, descriptor arguments, role tables, signature mirrors, or another record of facts already present in its declaration. A functor-owned constructor conversion states different data: how one structure constructs its selected ancestor implementation. |
...
| `POL-LEAF-054` | Keep kernel concerns out of every leaf import, decorator, annotation, signature, class attribute, and method body. Leaves state local role data and selected-functor constructor conversions. They do not inspect structural paths or control compilation, refinement, caching, or dispatch. |
...
| ID | Policy |
| --- | --- |
| `POL-KERNEL-001` | The kernel owns complete structural inheritance for objects, elements, and morphisms. This includes controlled role MROs, constructor composition, public functor images, route identity checks, and canonical caches. |
...
| `POL-KERNEL-003` | A leaf functor states its immediate mathematical action on objects and morphisms and the typed construction-input conversions required when that functor is selected. Its action on generalized elements is derived by applying the morphism action to each element's defining morphism. It never implements route traversal or cache management. |
...
| `POL-KERNEL-010` | Keep dynamic role construction and copied method installation inside the kernel. Expose the resulting typed mathematical surface through ordinary Python inheritance, without wrappers or repeated reflection in theory code. |
...
| `POL-KERNEL-017` | The kernel alone discovers, composes, and traverses structural routes and invokes image transport. Theory code never sees these operations. |
| `POL-KERNEL-018` | Make each inherited method callable directly on every structural descendant. Its ordinary declaration executes through the compiled role MRO after the functor-owned constructor program initializes its declaring state. |
...
| `POL-KERNEL-020` | Copy each local executable declaration into its compiled role. Obtain inherited operations only from compiled ancestor roles. Never route a locally owned operation into Sage or another engine or match it to an engine method by name. |
| `POL-KERNEL-021` | Derive a method receiver's role from its owning local object, element, or morphism implementation declaration. Derive parameter and result roles from their exact mathematical types. Derive call shape from the Python signature. Never require a leaf to restate these facts. |
| `POL-KERNEL-022` | Use mathematical roles to type public functor images, constructor conversions, and canonical image relations. Never relabel a category, object, element, morphism, or mathematical collection as a plain value to suppress an exact role obligation. |
...
| `POL-KERNEL-028` | Keep each `DeclaredObjectType`, `DeclaredElementType`, and `DeclaredMorphismType` distinct from its compiled public role. Build the public role with Sage's dynamic-class technique. Copy each local member except `__init__` onto the controlled compiled-ancestor bases. Rebind each copied `__class__` closure to the compiled class. Retain the rebound local initializer as the node initializer. Install one generated `__init__` wrapper. The exact MRO contains one compiled class per reachable role node, then the kernel base. |
| `POL-KERNEL-029` | Allocate the public role instance first. Create its root construction input with that value as canonical image. Before local initialization starts, traverse the structural graph and compute one exact input for each reachable role node. Each input contains that node's role identity and local datum. Check all routes to a common node by canonical-image and input identity. Activate a role-specific construction context. Each generated class wrapper passes only its node's datum to its rebound local constructor. Each local constructor initializes only its new state and calls `super().__init__()` once. A node without a local initializer advances to the next C3 initializer. C3 initializes each node once, even when adjacent classes have no structural edge. The kernel base consumes only the root input's identity. |
...
| `POL-LAYOUT-004` | Quarantine the non-mathematical implementation kernel in its own subtree. Dynamic role compilation, constructor composition, route checks, and other standard Python machinery belong behind this boundary and never mix with mathematical theory code. |
...
Place dynamic role compilation, constructor composition, caches, registration, and backend conversion in private infrastructure modules outside those mathematical units.
...
| `POL-FUN-003` | Only functors selected in `structure_functors()` contribute inherited public implementations. This includes compiled role bases and constructor conversions. Selection affects compiler behavior only; it does not change the selected object's mathematical type in `Fun`. |
...
| `POL-FUN-006` | Use functor composition to propagate role implementations and their constructor data. Do not add a separate inheritance or propagation registry. |
...
| `POL-FUN-019` | Define `C.Products()`, `C.Coproducts()`, `C.TensorProducts()`, and analogous named constructions as categories of chosen apexes with their presentations. Each object retains its input diagram, defining morphisms, and universal maps. Keep these categories distinct from the full replete subcategory `C.ImagesOfFunctor(F)`, which records only the existential image property. |
| `POL-FUN-020` | Lift an inherited universal construction through the selected structural functor. Retain its chosen presentation, apex, universal morphisms, and comparison map instead of reconstructing a parallel result. |
...
| `POL-FUN-035` | A selected functor retains one pure typed construction-input conversion for each object, element, and morphism role that its codomain initializes. An input contains the source node's role identity and typed construction datum. Each canonical value retains one root input. A conversion constructs the canonical target through its category and returns that value's retained input; it never allocates a parallel input record. The same conversion implements public functor application. All routes to one node return the same image and input by identity. The conversion does not inspect a partly initialized value. Identity and composite functors retain the corresponding identity and composite conversions. |
...
presentation. It retains `D`, its `product_projection(i)` morphisms, and its universal
...
| `POL-TYPE-028` | Preserve the exact receiver, positional-parameter, keyword-parameter, and result types of every method copied into a compiled role. `Callable[..., Any]` and `Callable[..., object]` are forbidden. |
...
For a poset module, bind `PosetElement = _POSETS.ElementType` after `_POSETS` is
constructed. Type `is_sup(x: PosetElement)` accordingly.
...
| `POL-TEST-006` | Test object, element, and morphism inheritance by constructing values through the real category compiler and calling inherited public operations. Route records, generated type identities, fields, and caches do not prove that public surface. |
