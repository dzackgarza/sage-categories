...
| `POL-SCOPE-012` | Kernel code implements category-independent class compilation, structure-functor construction-input conversion, dynamic types, and constructor composition through Sage's controlled linearization. Each named functor owns its public images. The kernel contains no category-specific mathematics. |
...
| `POL-CAT-012` | Construct `C.ObjectType`, `C.ElementType`, and `C.MorphismType` with Sage dynamic classes and Sage's controlled linearization. A shared ancestor class occurs once in the MRO and its initializer runs once. The kernel does not enumerate paths to that class, construct competing inputs for it, or compare data from different paths. Each named functor keeps its own public images. |
...
| ID | Policy |
| --- | --- |
| `POL-KERNEL-001` | The kernel owns category-independent class compilation and structure-functor constructor conversion for objects, elements, and morphisms. It constructs dynamic bases from immediate targets, supplies typed construction inputs, and follows Sage's controlled initializer order. Each named functor owns its public images. |
...
| `POL-KERNEL-017` | The kernel constructs each compiled class from its immediate structure-functor target classes and their typed constructor conversions. Sage dynamic classes and Sage's controlled linearization determine the reachable MRO. Neither kernel nor theory code enumerates functor paths or compares data assigned to them. Each named functor constructs its own public images. |
