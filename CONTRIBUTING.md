...
| `POL-LEAF-008` | Confine private-field access to constructors and genuinely new functor maps that cannot recover their required defining data through owned semantic interfaces. A standard functor `cat_kernel` builds never requires leaf access code (D10, D175). |
