...
The active A/B tables in [TODO.md](TODO.md) own execution routing and milestone acceptance, with [DECISIONS.md](DECISIONS.md) supplying the later architectural direction.
The archived native remediation plan retains allocation and preservation obligations; it is not the current work selector.

## Work selection and issue capture

| Policy identifier | Name and contract |
| --- | --- |
| `POL-WORK-001` | **Follow mathematical dependencies.** Use the active A/B DAG and contract links in [TODO.md](TODO.md). Give work stable IDs and explicit immediate prerequisites. Preserve exact objects, maps, hypotheses and complete consumer obligations when adjusting edges; verify references, acyclicity, milestone reachability and historical separation through `just plan-state`. Review semantic coverage against the decisions and full topic contracts separately. Shared files and subject priority are not mathematical dependencies. |
...
| `POL-WORK-003` | **Preserve concurrent records.** Reread the target entry before editing; preserve other workers' changes and staged files. Establish that another writer holds the tree before applying that preservation; see `POL-WORK-005`. Commit only the intended paths. Keep complaint evidence, execution dependencies, and enduring mathematical contracts at their respective owners. |
| `POL-WORK-004` | **Verify a waiting job is alive.** [Context and attention](AGENTS.md#context-and-attention) requires a process check and evidence of advancing output before continuing to wait on a long-running job. No live process and no new output is a dead session whose result will never arrive; relaunch the operation rather than polling it, and do not report the unit blocked or waiting on it. |
| `POL-WORK-005` | **Own the working tree.** [Session continuity][session continuity] establishes the writer count from `git worktree list`, `git branch --show-current`, and `git status --short`. A single worktree on a single checked-out branch has one writer; its uncommitted and staged changes belong to that writer or its predecessor, to finish, validate and bank. Assumed concurrency is not evidence of another agent. |
| `POL-WORK-006` | **Bank before waiting.** [Gates, fronts, and claims](AGENTS.md#gates-fronts-and-claims) forbids holding staged or modified work unbanked across a long-running job. Commit each substantive unit at its own boundary and name the pending job in the commit message under the recorded known-red exception. |
| `POL-WORK-007` | **Retire a delegation worktree.** [Delegation](AGENTS.md#delegation) makes the creating agent responsible for removing an isolated worktree once the integrated unit is committed and the delegate's commits are reachable from the branch. A stale entry in `git worktree list` reads as a second live writer under `POL-WORK-005`. Establish a worktree you did not create from its status, its ancestry, and live processes under its path before removing it; preserve and report one that fails any of those readings. |
| `POL-WORK-008` | **Keep bulk scratch project-owned.** Repository execution must not spill large test, build, package, or cache state into global `/tmp` or `~/.cache`. Use the repository's ignored `.tmp/` or the retained `/dev/shm` runtime staging, respect explicit existing runtime bindings, and remove disposable scratch at the unit boundary. A clean-validation need does not justify another full checkout. |
