...
| `POL-LEAF-033` | A concrete category that implements `C.P()` uses Sage's private axiom registration. It defines the property meaning and registers exact SymPy handlers. It can declare its nested implementation classes and supported constructors. `Cat` owns the monomorphism and inverse-image categories. `cat_kernel` generates `is_P()`; the kernel performs same-object refinement. |
