...
| `POL-SCOPE-009` | Judge an architectural claim from the live method owners, declared functors, compiled class graph, construction-input conversions, constructor obligations, and universal data. Agent reports, passing tests, route metadata, generated type identities, and runtime output cannot replace that inspection. |
...
| `POL-SCOPE-011` | Leaf code is theory code for one category. It states only that category's new data, operations, declared functors, constructors, and lifts. |
...
| `POL-MATH-050` | Use standard mathematical and Python terms. Define a project-specific descriptive phrase at its first use. |
| `POL-MATH-051` | Support each mathematical term by its standard definition. Repository usage does not establish mathematical meaning. |
| `POL-MATH-052` | If the standard mathematics does not contain a proposed object or operation, report the missing construction. Do not create runtime metadata to simulate it. |
| `POL-MATH-053` | Name an implementation class by its exact mathematical kind, such as `C.ObjectType`. Use plain descriptive text for compiler mechanics. |
...
| `POL-CAT-005` | A leaf category knows only itself and its immediate declared functors. |
...
| `POL-CAT-016` | Derive supercategory information from the declared functors. Do not maintain a second inheritance registry. |
...
| `POL-CAT-052` | Make generic category constructors propagate through declared functors. A descendant category supplies no boilerplate merely to form the inherited construction subcategory. |
...
| `POL-CAT-054` | Declare every relation between categories by a declared functor, including a subcategory monomorphism, identity, or other trivial functor. A category without these functors is disconnected from the owned category graph. |
| `POL-CAT-055` | Treat a failed declared functor or method compiler as a foundational defect. Its failure does not permit explicit subclassing or another inheritance path. |
...
| `POL-CAT-058` | Compile `ElementType` inheritance from declared functors by the same mechanism used for `ObjectType` and `MorphismType`. A subcategory never reuses another category's element implementation type. |
...
| `POL-CAT-077` | Determine method ownership from its definition on the category-owned implementation class and the declared functors. No decorator, marker, annotation payload, registry entry, or descriptor argument can create mathematical ownership or repair a missing category declaration. |
...
| `POL-CAT-096` | A functor has an image; a category does not. The image of `x` is `F.on_object(x)` for a named functor `F`, and `F.on_morphism(f)` for a morphism. Provide no operation that asks a category for the image of a value at that category: a leaf may declare several functors into one target, so naming only the target leaves the functor unstated. The identity agreement of `POL-CAT-066` and decisions 4 and 14 of `specs/resolution.md` is a fact about the declared functors. It creates no accessor on the target, and single-valuedness that holds only because disagreement raises is not a mathematical selection. |
