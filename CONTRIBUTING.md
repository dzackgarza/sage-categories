...
A predicate application is a symbolic proposition with typed mathematical arguments, such as `order_preserving(f)`.
...
| `POL-ASSUME-009` | Keep engine predicate representations inside the backend. Public APIs use owned categories, propositions, the standard session-level `assume()` operation, and `Decision`. |
