...
| `POL-CAT-003` | Construct `C.ObjectType`, `C.ElementType`, and `C.MorphismType` from their exact mathematical owners. Share a base only when the mathematical containment states it. |
...
| `POL-CAT-012` | During source construction, compare the constructor data supplied by all selected paths to one `D.ObjectType`, `D.ElementType`, or `D.MorphismType`. A mismatch raises a construction-defect error naming the source construction, both paths, and the target class. Each named functor keeps its own public images. Natural transformations never identify constructor data or public images. |
...
| `POL-LEAF-059` | Specify every leaf category with `C.ObjectType`, `C.ElementType`, and `C.MorphismType` directly. The kernel constructs these classes dynamically. Declare each property axiom once and let Sage's axiom mechanism generate the abstract property subcategory. A separate concrete category specifies its own `ObjectType`, `ElementType`, and `MorphismType` and connects itself to that axiom subcategory with `_base_category_class_and_axiom`. Each generated `D.P()` refinement uses that one axiom declaration (`POL-CAT-084`). |
...
| `POL-KERNEL-028` | The kernel constructs `C.ObjectType`, `C.ElementType`, and `C.MorphismType` dynamically. An object of `C` is an instance of `D.ObjectType` for every selected `F: C -> D`, and it carries the private data that methods of `D.ObjectType` need. Apply the same rule to `C.ElementType` and `C.MorphismType` when the selected functor supplies the required conversions. The class-construction mechanism is not fixed here (`POL-DOC-017`). |
...
| `POL-API-025` | Use only the exact names `C.ObjectType`, `C.ElementType`, and `C.MorphismType`. A category specifies these classes, and the kernel constructs them dynamically from the selected functors. For each selected `F: C -> D`, `C.ObjectType` inherits `D.ObjectType`. `C.ElementType` and `C.MorphismType` inherit the corresponding classes when `F` supplies their conversions. A leaf never constructs this inheritance. |
