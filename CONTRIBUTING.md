...
| `POL-LEAF-033` | A property-subcategory leaf declares its predicate, its subcategory monomorphism, and its trusted property constructor. The public `is_*()` method always returns the applied predicate. Category placement makes `ask()` return `True`. The leaf declares one constructor only. |
...
| `POL-FUN-002` | A functor owns its object and morphism actions. For a point `t: 1_C -> X`, derive `F.on_element(t)` from `F.on_morphism(t)` and a supplied morphism `c_F: 1_D -> F(1_C)`. The composite is a point `1_D -> F(X)`. For a generalized element `t: T -> X`, the morphism action gives `F(t): F(T) -> F(X)`; this is not an `ElementType` value unless its domain is the terminal object. `Cat().ElementType` covers only the point case `* -> C`. |
...
| `POL-API-027` | During version 1, add no method for an operation expressible as one or two lines of public compositional code. Expose the defining mathematical data at its owner, then compose existing operations at the call site. |
