...
| `POL-CAT-083` | Construct or refine a distinguished value directly in each established ambient or property category. Use `Cat().Point(X)` only for the retained one-object category and its diagrams. A functor from that category causes no placement or inheritance. When a category `C` is an object of a structured category `D`, direct placement supplies the applicable `D.ObjectType` surface to `C` and the `D.ElementType` surface to `C.ObjectType`. |
