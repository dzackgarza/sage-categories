...
| `POL-CAT-092` | Define `Products()`, `Coproducts()`, `Subobjects(X)`, `Superobjects(X)`, `CoveringObjects(X)`, `CoveredObjects(X)`, `Limits(I)`, and `Colimits(I)` at the `Cat` level. For `X in C`, `C.Subobjects(X)` is the full subcategory of `C.SliceOver(X)` on monomorphisms, and `C.Superobjects(X)` is the full subcategory of `C.CosliceUnder(X)` on monomorphisms. `C.CoveringObjects(X)` is the full subcategory of `C.SliceOver(X)` on epimorphisms, and `C.CoveredObjects(X)` is the full subcategory of `C.CosliceUnder(X)` on epimorphisms. The ambient category in the call fixes the role of `X` when the same value belongs to more than one category. `Sets().Subobjects(X).from_predicate(predicate)` constructs the set subobject selected by a predicate. Restricting a leaf's structure to a subobject is leaf work (`POL-LEAF-060`). Apply the generic constructions to every category, including `Cat()` itself. Each limit or colimit family is indexed by one supplied shape `I in Cat()`; `C.Limits(Cat())` never denotes limits over all small shapes. A discrete diagram is a rule `i |-> X_i` on an index set; a Python sequence denotes the diagram over `Discrete([n])`. Bicompleteness and cartesian closure of `Cat()` are trusted declarations attached to the constructors the kernel supplies; they assert nothing about a shape or category not supplied to a constructor. |
...
| `POL-LEAF-060` | Restricting structure to a subobject is leaf work, and the compiler never learns it. For a poset presented as `(X, R)`, `Sets().Subobjects(X).from_predicate(predicate)` constructs only the selected set and its monomorphism into `X`. `PartiallyOrderedSets().Subobjects(P)` owns subobjects of the poset `P`; its construction retains the restricted order and the defining monomorphism in `PartiallyOrderedSets()`. The ordering subtree decides whether a result lies in `PartiallyOrderedSets()` or `TotallyOrderedSets()`. The categorical theorem that transports a monomorphism between the two categories belongs to the leaf that states it. A general category never contains a specific leaf's restriction rule. |
...
| `POL-API-002` | During version 1, give each operation one mathematical owner, one canonical public spelling, and one public export. A public method declared on an owned implementation type is that operation, not a forwarding entry point to another spelling. A standard mathematical operator invokes the same owned implementation. Add secondary public spellings only after version 1. |
