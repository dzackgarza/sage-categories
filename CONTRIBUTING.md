...
| `POL-CAT-092` | Define `Products()`, `Coproducts()`, `Subobjects()`, `Superobjects()`, `CoveringObjects()`, `CoveredObjects()`, `Limits(I)`, `Colimits(I)`, and the standard dual constructions at the `Cat` level. Their fixed-object fibres are `C.Subobjects()(X)`, `C.Superobjects()(X)`, `C.CoveringObjects()(X)`, and `C.CoveredObjects()(X)`. `Cat().Subobjects()` gives subcategories, and `Cat().Products().Subobjects()` gives subcategories of a product category. `X.subset_from(predicate)` is the `Sets()`-owned predicate-subobject operation and returns an object of `Sets().Subobjects()(X)`. Restricting a leaf's structure to a subobject is leaf work (`POL-LEAF-060`). Apply the generic families to every category, including `Cat()` itself. Each limit or colimit family is indexed by one supplied shape `I in Cat()`; `C.Limits(Cat())` never denotes limits over all small shapes. A discrete diagram is a rule `i |-> X_i` on an index set; a Python sequence is the convenience form over `Discrete([n])`. Bicompleteness and cartesian closure of `Cat()` are trusted declarations attached to the constructors the kernel supplies; they assert nothing about a shape or category not supplied to a constructor. |
...
| `POL-CAT-094` | A subobject of `P` is an object `S` with a monomorphism `j: S -> P`; there is no second notion and no separate name for a representative. The equivalence-class formulation matters only when deciding whether two subobjects are equal, which is a predicate. Let `P` be a product and let `j: S -> P` be such a monomorphism. Make the corresponding object of `Cat().Products().Subobjects()` retain `j`, and read `P` as its codomain. Define its `product_projection(i)` as `P.product_projection(i) after j`. Thus every such object has all component functors. Do not ask a leaf to repeat their maps. |
...
| `POL-LEAF-060` | Restricting structure to a subobject is leaf work, and the compiler never learns it. `X.subset_from(predicate)` on a poset presented as `(X, R)` returns a subset of `X` and lifts nothing: expecting a poset back would make `Sets()` know how to handle posets, which reverses the flow of ownership. `Sets()` is written once and never touched again. A leaf that wants subobjects in its own category declares its own method — `X.sub_poset(...)` — or overrides the inherited one, calling `super()` and then restricting the order itself. It is the ordering subtree's business to know how a partial order restricts, and to decide whether its result lands in `PartiallyOrderedSets()` or `TotallyOrderedSets()`. Neighbouring subtrees may let concerns flow both ways; a general subtree never carries a specific one's. The categorical fact behind this — that the adjunction between the two categories preserves monomorphisms — belongs to the leaf that states it, never to the kernel. A kernel that ensures subsets of a poset are posets, with no mechanism that generalizes beyond posets, is defective even when it works. |
