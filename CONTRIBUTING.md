...
| `POL-CAT-083` | Represent a distinguished named object `X` by a category class, a completely abstract new category. Every category class is a point in `Cat` automatically. Register `X` as a point in `D` by adding the structure functor `D.Point()`, the arrow `* -> D`, to that class; `D.Point()` is the arrow `* -> D` selecting `X`; the kernel carries placement and inheritance along the inclusion of `<X>`, the smallest full subcategory of `D` containing `X` whose inclusion is an isofibration, declared in `Fun(<X>, D).Monomorphisms().Isofibrations()` (`POL-FUN-036`, D167, D170), and it supplies the codomain's surfaces through the categorical level shift, `D.ObjectType` to `X` itself and `D.ElementType` to `X.ObjectType`, so every object of the category `X` is an element of the set `X` (D128, D154, D161). |
...
| `POL-LEAF-033` | A concrete category that implements `C.P()` uses Sage's private axiom registration. It defines the property meaning and registers exact SymPy handlers. It can declare its nested implementation classes and supported constructors. `Cat` owns the predicate meaning and the inverse-image categories; `cat_kernel` builds `C.P()` with its monomorphism and generates `is_P()`; the kernel performs same-object refinement (D175). |
...
| `POL-KERNEL-033` | Review property-class binding against [specs/resolution.md](specs/resolution.md) and [specs/property-refinement.md](specs/property-refinement.md): Sage binds implementation classes, `Cat` owns the predicate and the inverse images, and `cat_kernel` builds the property category with its monomorphism (D148, D175). |
