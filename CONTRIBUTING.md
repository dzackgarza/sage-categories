...
| `POL-MATH-034` | Give every mathematical truth question one category-owned predicate meaning and one SymPy proposition: a comparison of typed-query results with objects of their result category, such as `X.cardinality() < aleph0`, or the application of a SymPy `Predicate` the owner defines when no existing method supplies the proposition (D18, D148). Represent every partial value question as a category-owned typed query with one exact result category. A typed value is not a Boolean atom; a comparison of typed-query results is a proposition. Forming either application performs no evaluation. Only `ask()` returns `True`, `False`, an owned result, or Sage `Unknown`. |
...
| `POL-MATH-054` | Reserve "the kernel owns X" for machinery, and name the implementer whenever `X` is a mathematical object. `Cat`, `Mor(n, C)` and `Fun(C, D)` are defined by this repository and read as mathematics; the kernel realizes them and no leaf redefines them. The property subcategories and the subcategory inclusions they carry are `cat_kernel`'s, which builds each from its axiom declaration; a standard functor with no axiom declaration, such as a point arrow or a product projection, is `Cat`'s (D10, D148, D175). This row's directive half read "write the kernel implements X" of those two as well, which is the rule that produced eleven sentences giving one fact two owners; naming them apart is what stops the next one. The word matters because ownership carries two meanings here - who implements a thing, and whether a thing is inside the black box - and using it for both puts mathematical objects outside mathematical audit (`POL-MATH-039`). Category-owned implementation classes keep the word: a category does own its `ObjectType`, `ElementType`, and `MorphismType`. |
...
| `POL-ONT-003` | Evaluate membership in a property subcategory `X in C.P()` strictly through the three-valued proposition `ask(X.is_P())`, the `is_P()` `cat_kernel` generates from the axiom (D148, D175); membership in a structure category is placement by construction (D150). Never treat `in` as a container item check or query it on raw carrier data. |
...
| `POL-ONT-006` | A property handler decides by `match`/`case` the cases its exact rule can decide and returns `None` for every other case; a new decidable case extends the `match`. A case may ask an exact subquestion, a proposition evaluated through `ask()`; SymPy owns cycle safety (D151). `X in C` is the forced Boolean protocol over that evaluation (`POL-ONT-003`). |
...
| `POL-ONT-008` | Ban catch-all constructors, fallback heuristics, optional arguments, and default values. A constructor takes construction data. A general category's constructor `C(datum)` dispatches by `match`/`case` on the supplied datum into the proper subcategory's constructor, so `Sets({1, 2, 3})` constructs a finite set without the caller naming `Sets().Finite()`. Each specific route has its own separate, explicitly named total constructor (D150). |
...
- **The Internalized Reality (`specs/undecidable-properties.md`)**:
  - In this architecture, `X in C.P()` is the categorical membership proposition:
    $$\mathrm{ask}(X.\mathrm{is\_P}())$$
...
- **The Internalized Reality (`specs/undecidable-properties.md`, `specs/sets.md`)**:
  - `ambient in category` is the forced Boolean protocol over `ask()`; a handler asks an exact subquestion through `ask()` on the proposition itself, and SymPy owns cycle safety (D151).
...
| **Deduction Handlers** | Call `in` inside a handler | `match`/`case` on the cases the exact rule decides, exact subquestions through `ask()`; `None` otherwise | `POL-ONT-006` |
...
| `POL-CAT-038` | Make the category call `C.P()(...)` the standard public trust boundary for construction into a property subcategory. `C.P()` has exactly the constructors of `C`, through `cat_kernel`'s functor `C.P() -> C` (D175), and dispatches to the exact constructor for the supplied construction data. Construction in `C.P()` asserts the defining property. A value already constructed is placed by `assume(X.is_P())` (D21, D150). |
...
| `POL-CAT-083` | Represent a distinguished named object `X` by a category class, a completely abstract new category. Every category class is a point in `Cat` automatically. Register `X` as a point in `D` by adding the structure functor `D.Point()`, the arrow `* -> D`, to that class; `D.Point()` is the arrow `* -> D` selecting `X`; `cat_kernel` reads the declaration and the kernel then carries placement and inheritance along the inclusion of `<X>`, the smallest full subcategory of `D` containing `X` whose inclusion is an isofibration, declared in `Fun(<X>, D).Monomorphisms().Isofibrations()` (`POL-FUN-036`, D167, D170), and it supplies the codomain's surfaces through the categorical level shift, `D.ObjectType` to `X` itself and `D.ElementType` to `X.ObjectType`, so every object of the category `X` is an element of the set `X` (D128, D154, D161). |
...
| `POL-CAT-088` | On `Cat().ObjectType`, define `C * D` as the product category `Cat().Products()(C, D)`, `C + D` as the coproduct category, and `D ** C` as `Fun(C, D)`. On `Cat().ElementType`, supply categorical product, coproduct, biproduct, and exponential operators as defaults. A category-owned implementation can override a default when standard notation for its objects names a different declared algebraic operation. The explicit categorical constructions remain `C.Products()`, `C.Coproducts()`, `C.Biproducts()`, and the named exponential construction; the one product pattern at every level is `X * Y = C.Products()(X, Y) = (C * C)(X, Y)`, and the specifications spell the general case `C.Products()(X, Y)` (D149). Local declarations win through the ordinary compiled MRO. A pair of `X in C` and `Y in D` is `(C * D)(X, Y)`, an object of `C * D`; a product in `C` is never formed from `C * D`. Each universal construction retains its defining morphisms. |
| `POL-CAT-089` | Define `Mor(C)` once at the `Cat` level for every `C in Cat()` (D173, `POL-CAT-023`). Its objects are the morphisms of `C`, and its morphisms are the 2-morphisms of `C`; for a 1-category it is discrete. Endpoint application `Mor(C)(A, B)` is the full subcategory on the morphisms `A -> B`, one cached object per pair. Obtain `Fun = Mor(Cat())` from the same construction: its objects are functors, and `Fun(C, D) = Mor(Cat())(C, D)` has natural transformations as morphisms. The category whose objects are the morphisms of `C` and whose morphisms are commuting squares is the functor category `Fun([1], C)` from the walking arrow, with evaluation functors `Fun([1], C).ev(0)` and `Fun([1], C).ev(1)`, each `Fun([1], C) -> C` (D157); it is not a primitive. |
| `POL-CAT-090` | Define `Mor(Cat()).Full()`, `.Faithful()`, `.FullyFaithful()`, `.EssentiallySurjective()`, and `.Equivalences()` through the axiom property-subcategory mechanism. `cat_kernel` generates their specified `is_*()` applications on `Cat().MorphismType` from those axiom declarations. Each returns the owned containment proposition. Direct construction, assumptions, category placement, and declared subcategory containments use the standard same-object refinement path. |
...
| `POL-CAT-095` | Present `C.SliceOver(x)` as the pullback in `Cat()` of `Fun([1], C).ev(1)` along `x: * -> C`, and `C.CosliceUnder(x)` as the pullback of `Fun([1], C).ev(0)` along `x`. More generally, `Comma(F, G)` is the pullback of `(ev(0), ev(1)): Fun([1], C) -> C * C` along `F * G`. Each retains its pullback projection, `C.SliceOver(x).projection()` or `C.CosliceUnder(x).projection()`; the varying object is its composite with `Fun([1], C).ev(0)` or `Fun([1], C).ev(1)`, and a composite is `G * F` (D157). Two distinct functors carry distinct lift data: `Fun([1], C).ev(1)` is a fibration when `C` has pullbacks, with the cartesian lift of `f: y -> x` at `p: z -> x` given by pullback; the fixed slice projection `C.SliceOver(x) -> C` is a discrete fibration for every `C`, with the cartesian lift of `f: y -> z` at `(z, p)` given by precomposition, `(y, p compose f) -> (z, p)`. Retain each lift at its construction owner. |
...
| `POL-LEAF-004` | A constructor takes construction data only. Placement of an already constructed value is `assume(X.is_P())`; a leaf writer writes one way to do each thing, as a named method on the relevant object, with no shortcut, alias, or convenience (D150). |
...
| `POL-LEAF-008` | Confine private-field access to constructors and genuinely new functor maps that cannot recover their required defining data through owned semantic interfaces. A standard functor never requires leaf access code, whether `cat_kernel` builds it from an axiom declaration or `Cat` constructs it (D10, D175). |
...
| `POL-LEAF-011` | When a named leaf functor creates limits of shape `I`, construct it into `Fun(C, D).CreatesLimits(I)` at its declaration; a functor's theorem is the property subcategory it is constructed into (D158). A structure functor is defined by the leaf with its two actions and constructed into the strongest property subcategory of `Fun(C, D)` that states what is known about it, `Fun(Posets(), Sets()).Fibrations().CreatesLimits(Discrete)(on_object, on_morphism)` for the poset leaf (D08, D161, D162). State only the leaf mathematics that establishes this property. The generic creates-limits construction supplies the lifted cone and universal morphisms ([poset-products template](specs/poset-products-minimal-template.py)). If the functor does not create those limits, a different leaf construction must name its actual mathematical source. |
...
| `POL-LEAF-033` | A concrete category that implements `C.P()` uses Sage's private axiom registration. It defines the property meaning and registers exact SymPy handlers. It can declare its nested implementation classes and supported constructors. The property category owns its predicate meaning (D142); `Cat` owns the inverse-image categories; `cat_kernel` builds `C.P()` with its monomorphism and generates `is_P()`; the kernel performs same-object refinement (D148, D175). |
...
| `POL-LEAF-064` | Red flag: a category that constructs a property or construction subcategory by hand, names it with a string, or patches an accessor onto it. An axiom is declared once at its owner, by its name and the proposition deciding membership, and `cat_kernel` routes `C.P()`, `C.Products()`, and `is_p()` from that declaration (D89, D133, D148, D175). |
...
| `POL-LEAF-068` | Red flag: a hand-written property application such as `is_finite()` on a leaf class. `cat_kernel` generates `is_p()` from the axiom declaration (D89, D135, D175). |
| `POL-LEAF-069` | Red flag: a datum-free constructor, or a point category built by hand. The leaf class of a named object is a point in `Cat` automatically and registers its object as a point in `D` by the structure functor `D.Point()` (D128, D135, D154, D161). |
| `POL-LEAF-070` | Red flag: a functor that computes its image selected without its two actions, or hand-written actions for a subcategory inclusion or point functor. A subcategory inclusion is declared as `Fun(S, T).Monomorphisms().Isofibrations()()`, the zero-argument call on the property category, with `.Full()` when `S` is full in `T`; a point functor is `D.Point()`; no actions are written for either; `cat_kernel` implements the axiom-derived subcategory inclusion, and `Cat` implements the point functor, which has no axiom declaration and whose constructing call is `C.Point()` (D10, D146, D154, D175). A functor that computes its image is declared by `Fun(C, D)(on_object, on_morphism)`, and the leaf writer's action is an entire Python function that feeds the source value into a constructor of `D` (D08, D10, D11, D123, D135). |
| `POL-LEAF-071` | Red flag: a retained projection rewritten as a leaf method. The defining construction retains its projections and universal data; the leaf selects them by the named method of the construction, `C.CosliceUnder(X).projection()`, `Fun(I, C).ev(i)`, `P.product_projection(i)`, and composes them as `G * F` (D09, D10, D135, D157). |
...
| `POL-LEAF-080` | A category class declares itself the implementation of a named category `x` (an axiom subcategory `D.P()`, a construction category `C.CosliceUnder(X)`, or any spelling) by selecting the identity functor of `x` as a structure functor, `structure_functors = [End_Cat(x).one(), ...]`; there is no bespoke binding field (D156). |
| `POL-LEAF-081` | Two structure functors whose targets both declare an axiom `P` define one `C.P()`: the pullbacks are equivalent, Sage's C3 order chooses the first, and coherence is assumed (D37, D159). |
...
| `POL-KERNEL-033` | Review property-class binding against [specs/resolution.md](specs/resolution.md) and [specs/property-refinement.md](specs/property-refinement.md): Sage binds implementation classes, the property category owns its predicate (D142), `Cat` owns the inverse images, and `cat_kernel` builds the property category with its monomorphism (D148, D175). |
...
| `POL-KERNEL-037` | Every red-flag shape in `POL-LEAF-063` through `POL-LEAF-079` names a capability the leaf had to supply for itself. The repair adds that capability at its owner — the kernel for threading, retention, refinement and placement, `Cat` for every operation the mathematics defines, `cat_kernel` for the axiom subcategories with their inclusions and generated predicates (D175) — and deletes the leaf wiring; a leaf-local patch that keeps the shape is rejected (D133, D173). |
| `POL-KERNEL-038` | The kernel states no mathematics. A kernel role class declares only the closed surface in [specs/resolution.md](specs/resolution.md) "The closed kernel surface": the class-compilation entry points, the construction-context initializers, the role tests, the functor-image-cache reads, and `__hash__`. A morphism's endpoints and its composition belong to `Mor(C).ObjectType`; a point's `parent`, `defining_morphism`, `category`, and equality belong to `Cat()`. A kernel module imports no module of `Cat`. The direction is kernel to `Cat` to leaves, and both halves are D132 checks (D130, D173). |
...
Its structure functor to `Sets()`, `(X, R) |-> X`, constructed into `Fun(Posets(), Sets()).Fibrations().CreatesLimits(Discrete)`, supplies membership, iteration, cardinality, elements, and set maps through the compiled interface (D161, D162, D163).
...
| `POL-FUN-036` | Propagate established placement from `S` to `T` along a functor that is both a monomorphism of `Cat()` and an isofibration, and along no other functor; placement is membership in a subcategory, so it needs the monomorphism. Inheritance of methods needs less: a faithful isofibration, which a structure functor such as `Groups() -> Sets()` is without being injective on objects (D167). A leaf declares a placement relation by constructing in `Fun(S, T).Monomorphisms().Isofibrations()`, a full subcategory adds `.Full()`, and a structure functor is constructed into `Fun(S, T).Isofibrations()` or a subcategory of it such as `Fibrations()`. Monicity is faithfulness with injectivity on objects, so one value is shared rather than copied. The isofibration condition, that every isomorphism of `T` at an object in the image lifts to `S`, is what makes strict membership respect the principle of equivalence; without it a skeleton would qualify, and a cardinal would be a set (D170). A subcategory is a subobject of `T` in `Cat()`, and no intrinsic choice of representative exists, so the leaf declares which monomorphism placement follows through the owning property category, and `cat_kernel` reads that declaration and trusts it (D175). For a point functor the declaration is written by `C.Point()` itself, in the call that constructs the arrow, into `Fun(*, C).Monomorphisms()` (D154, D161). The arrow is monic and faithful for every `C` and `X` and is not an isofibration; the inclusion `<X> -> C` it generates is. Its fullness is not declared: nothing decides it and nothing reads it. Never infer the relation from Python inheritance, shared storage, or a table of previously constructed functors. See `specs/functor.md`, "Monomorphisms of `Cat()` and placement". |
| `POL-FUN-028` | Name every functor by the construction that supplies it. Fixed endpoints, a source object presentation, or a collection of Python fields do not select an object of `Fun(C, D)`. Use the exact projection, inclusion, evaluation, composite, or right adjoint to a named free functor. For `Posets() -> Sets()`, the leaf defines the fibration `(X, R) |-> X` by its two actions, the underlying set and never the relation, constructed into `Fun(Posets(), Sets()).Fibrations().CreatesLimits(Discrete)` (D162, D163). |
...
| `POL-FUN-031` | Form slice and coslice projections from the pullback projections `C.SliceOver(x).projection()` and `C.CosliceUnder(x).projection()` to `Fun([1], C)`, composed with the evaluation functors `Fun([1], C).ev(0)` and `Fun([1], C).ev(1)` (D157). `Fun([1], C).ev(1)` is a fibration when `C` has pullbacks; the fixed slice projection `C.SliceOver(x) -> C` is a discrete fibration by precomposition for every `C`. State each lift at its construction owner; do not infer it from object fields. |
...
- `Fun(I, C)` supplies its evaluation functors `Fun(I, C).ev(i)`;
- a slice or coslice supplies its projection, `C.SliceOver(x).projection()` or `C.CosliceUnder(x).projection()`, to its defining morphism, and the composite with evaluation to its varying object;
...
- ordinary composition `G * F`, the morphism composition of `Cat`, combines these maps into named composite functors (D157).
...
subcategory, whose inclusion is an isofibration, records no selected diagram or projections.

For objects `X, Y`, `C.Products()(X, Y)` selects `p in LimitCones((X, Y))` and returns
...
| `POL-TYPE-004` | Annotate the candidate parameter of every `__eq__` as `EqualityInput` and of every `__contains__` as `ContainmentInput`, the two aliases of `Any` declared once; these two parameter positions are the only permitted uses of `Any` (D131). |
...
| `POL-TYPE-014` | The candidate of `__eq__` is `EqualityInput` and of `__contains__` is `ContainmentInput`, each an alias of `Any` declared once: never `object`, never a structural or invented indirected type (D131). |
| `POL-TYPE-015` | Never create a type such as `MembershipInput` that models the candidate accepted by `__eq__` or `__contains__`; the two aliases name the position, not a model of the input (D131). |
...
The special-method signatures are `__eq__(self, candidate: EqualityInput)` and `__contains__(self, candidate: ContainmentInput)`, with `EqualityInput = Any` and `ContainmentInput = Any` declared once.
Use those two aliases at those two special-method boundaries and nowhere else.
...
| `POL-DOC-024` | An agent never reports that it understood, learned, or internalized a decision; it names the document that now records it. A decision that is not written in its owning document does not exist for the next session (D134). |
...
| `POL-DOC-029` | `just plan-state` at the push tier reads the vault cards and fails unless exactly one phase is `in-progress` across every plan, every `complete` phase carries `Accepted revision: <hash>`, every open or complete phase has complete prerequisites, and no production leaf package is in `src` while the core plan is open. A plan written after observed agent failure carries the gate that stops that failure (D136, D137, D138). |
