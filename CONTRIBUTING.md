...
| `POL-CAT-033` | Define a subcategory only for a genuine mathematical property, structure, or declared algebraic operation. Additive and multiplicative refinements can select one spelling for a neutral operation. Never define a subcategory only to select, store, or expose an implementation. |
...
| `POL-LEAF-033` | A property-subcategory leaf declares `predicate_name`, `predicate_owner`, its containment predicate, its subcategory monomorphism, and its trusted property constructor. The kernel derives the public application. Category placement makes `ask()` return `True`. The leaf declares one constructor only. |
