...
| `POL-ONT-003` | Evaluate membership in a property subcategory `X in C.P()` strictly through the three-valued proposition `ask(X.is_P())`, the `is_P()` the kernel generates from the axiom (D148); membership in a structure category is placement by construction (D150). Never treat `in` as a container item check or query it on raw carrier data. |
...
| `POL-LEAF-064` | Red flag: a category that constructs a property or construction subcategory by hand, names it with a string, or patches an accessor onto it. An axiom is declared once at its owner, by its name and the proposition deciding membership, and the kernel routes `C.P()`, `C.Products()`, and `is_p()` from that declaration (D89, D133, D148). |
