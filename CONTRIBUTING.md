...
| `POL-FUN-019` | Define `C.Products()`, `C.Coproducts()`, `C.TensorProducts()`, and analogous named constructions as categories of chosen presentations. For each input diagram `D`, their constructor returns a presentation `P_D` that retains its canonical apex `A_D`, defining morphisms, and universal maps. Their selected apex functor sends `P_D` to `A_D` in `C`. Keep these categories distinct from the full replete subcategory `C.ImagesOfFunctor(F)`, which records only the existential image property. |
...
| `POL-FUN-035` | A selected functor retains one pure typed construction-input conversion for each object and morphism role that its codomain initializes. An input contains the source node's role identity and typed construction datum. Each resulting canonical value retains one root input. A conversion constructs the canonical target through its category and returns that value's retained input. The same conversion implements the public object or morphism action. For an element `t: G_C -> X`, the public image is `F(t): F(G_C) -> F(X)`. The compiler derives a separate classical input by precomposing this image with the retained stage comparison `G_D -> F(G_C)`. These two generalized elements retain separate identities and cache entries. All routes to one node return the same image and input by identity. A conversion reads no partly initialized value. Identity and composite functors retain the corresponding conversions and stage comparisons. |
...
For each diagram `D`, an object `P_D` of `C.Products()` is the chosen product
presentation. It retains `D`, its canonical apex `A_D`, its `product_projection(i)`
morphisms, and its universal maps. Its selected apex functor maps `P_D` to `A_D` in
`C`. The apex lies in the essential image. The presentation category owns the chosen
diagram and universal data.
