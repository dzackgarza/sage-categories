...
| `POL-MATH-049` | State the current architecture in permanent documentation. Put former implementations and their reasons in Git history. |
| `POL-MATH-050` | Every technical term used in permanent documentation has a row in [specs/glossary.md](specs/glossary.md). A row is complete when it carries either an inspected external source with an exact locator, or one plain-English definition and the file and heading that states it. Writing a term with no complete row is the defect; the repair is to complete the row or to rewrite the sentence without the term. Never ask whether a term is standard or invented. Look it up. |
| `POL-MATH-051` | Support each mathematical term by its standard definition. Repository usage does not establish mathematical meaning: source, tests, plan text, review reports, and earlier policy rows were written by the same process under audit, so none of them is ever the content of a glossary row. External sources and the user establish meaning; this repository records it. |
| `POL-MATH-052` | If the standard mathematics does not contain a proposed object or operation, the thing being named is an implementation artifact: report the missing construction, open no glossary row, and create no runtime metadata to simulate it. |
| `POL-MATH-053` | Name an implementation class by its exact mathematical kind, such as `C.ObjectType`. Use plain descriptive text for compiler mechanics. A plain descriptive phrase is not a term and needs no glossary row: "the class compiled for objects of `C`" is admissible as written, while "the object role" is a term. |
...
| `POL-ASSUME-015` | Use no Boolean for a mathematical question anywhere in the package. A `bool` is honest only for an implementation fact that is two-valued by construction: established placement, a record of what was retained, the presence of a chosen enumeration. Everything else is a proposition, and `ask()` is the only thing that decides one. |
...
| `POL-CAT-002` | Use `ObjectType`, `ElementType`, and `MorphismType` for category-owned implementations. The kernel owns `Cat`; every category uses `Cat().ObjectType`, every functor uses `Cat().MorphismType`, and every natural transformation uses `Fun.MorphismType`. `Cat().ElementType` implements points `* -> C`, with terminal category `*`. These points are the actual objects of `C`, so every `C.ObjectType` inherits it. `C.ElementType` implements points `1_C -> X` of objects `X in C`. A generalized element `T -> X` is not an `ElementType` value unless `T = 1_C`; for `C in Cat()`, it is an object of `Fun(T, C)`. `C.MorphismType` is `Mor(C).ObjectType`. |
...
| `POL-CAT-056` | Apply the structural-functor framework to every functorial, universal, and morphism-based construction. Each construction creates and retains its distinct monomorphism, projection, or evaluation functors through `Fun(Source, Target)`. The endpoint pair never selects one functor. |
...
| `POL-CAT-098` | When objects carry a choice of extra data, name the category for the data, following Sage: `Modules(R).WithBasis()` is "the category of modules with a distinguished basis" (`sage/categories/modules_with_basis.py:179`), and `AlgebrasWithBasis`, `WithRealizations` (`sage/categories/with_realizations.py`), and the `FinitelyGenerated` family follow the same shape on the same axiom machinery as `Finite`. Use `With<Datum>`; never `Chosen<Anything>`. The test is whether the choice is retained data the public surface uses: a basis gives coordinates, an enumeration gives indexing, a generating set gives generators. Where it is, the subcategory is genuine structure and its constructor takes the datum. Where there is no bare object to equip, the pattern does not apply — a subobject *is* an object together with a monomorphism, so the family is `Subobjects()` (`POL-CAT-094`). Where a construction family already retains its construction, as every one here does, it is the equipped form and needs no adjective. |
...
- Limits, colimits, products, coproducts, tensor products, and direct sums declare their declared functors to the categories of their resulting objects.
...
| `POL-LEAF-059` | Declare every category as a class, including each property subcategory. Each category declares `ObjectType`, `ElementType`, and `MorphismType` directly as nested classes. The kernel compiles those same classes. A property subcategory uses Sage's general axiom mechanism: declare the axiom once, define the category class independently, and connect it to the ambient category and axiom with `_base_category_class_and_axiom`. |
| `POL-LEAF-001` | Integrate a new leaf category by supplying its declared functors to known categories. These functors are the complete inheritance declaration. |
| `POL-LEAF-002` | Make a leaf constructor accept only its strongest minimal semantic datum. Recover every weaker component through the datum's domains, codomains, ambient products, defining morphisms, and declared functors. Never require an underlying set, module, ring, or other ancestor object as a second argument when the supplied relation, morphism, form, or structure map already determines it. |
...
| `POL-LEAF-005` | Let the category compiler inherit the target categories' object, element, and morphism methods along declared functors. A leaf category defines no forwarding methods. |
| `POL-LEAF-006` | Treat a leaf implementation of an inherited operation as evidence of a missing declared functor, an incorrect functor image, or an operation placed at the wrong owner. |
| `POL-LEAF-007` | Permit a declared functor to land in a morphism category `Mor(C)` when the defining morphism determines the required inherited object data through its domain or codomain. |
...
| `POL-LEAF-011` | Lift an inherited construction to a leaf category by specifying only how the leaf's additional structure acts on the inherited result and morphisms. Make this lift compatible with the declared functors. |
...
| `POL-LEAF-014` | Ship and maintain a standard template for new leaf categories. The template contains only the category declaration, minimal constructor, declared functors, and sites for new methods. A template is design pseudocode that shows the required simplicity, so it is never imported, executed, type-checked, or held to a spelling criterion. It shows one way to write a leaf, chosen as a worked example. It does not fix the presentation of the mathematics it illustrates: many models of a poset are correct, and a template showing one of them does not make the others stale. A template is a defect only when it teaches a shape the kernel cannot compile. |
...
| `POL-LEAF-016` | After the declared functors are declared, automatically supply the complete applicable object, element, morphism, and construction interfaces from their target categories. |
...
| `POL-LEAF-018` | Do not implement an inherited category-owned mathematical operation in a leaf object. A local `__iter__`, `__contains__`, or `cardinality()` on a poset object duplicates the set interface instead of receiving it through the declared functor to `Sets()`. |
...
| `POL-LEAF-023` | Do not copy inherited storage, caches, or constructor arguments into a property refinement. A finite poset adds finite-poset operations and declares its subcategory monomorphisms. Same-object refinement preserves its underlying set, relation, elements, private realizations, and existing structural images. The refined dynamic MRO supplies inherited operations; the leaf never traverses a monomorphism to recover its own state. |
| `POL-LEAF-024` | A finished leaf contains its category, minimal defining data, new operations, immediate declared functors, and named constructors. It can also state leaf-specific lifts. |
...
| `POL-LEAF-032` | Treat declared functors as the complete inheritance program. A finished leaf contains no forwarding, descriptor, route, cache, wrapper, or type-repair boilerplate. |
...
| `POL-LEAF-037` | Never discover, inspect, compose, or traverse structural routes in a leaf. Declare immediate declared functors and use the resulting public inherited surface. |
...
Selected declared functors are executable inheritance declarations.
...
A lattice-specific cardinality implementation signals a missing or incorrect declared functor.
...
A new specialized algebra category should start from the leaf template, declare its declared functors to nearby algebra and module categories, and add only its new algebraic methods.
...
An enumeration is additional mathematical structure and therefore belongs to a separate category with its own declared functor to `TotallyOrderedSets()`.

## Mathematical encapsulation and repository layout

| ID | Policy |
| --- | --- |
| `POL-LAYOUT-001` | Keep a leaf subtree expressed in the language of its own category, its defining structure, and its immediate declared functors. Deeply underlying operations belong to the category that owns them. |
| `POL-LAYOUT-002` | Treat a reference to cardinality inside a lattice subtree as an ownership defect. Cardinality reaches lattice objects through their declared functors to modules and sets. |
...
| `POL-FUN-020` | Lift an inherited universal construction through the declared functor. Retain its chosen diagram, constructed object, universal morphisms, and comparison map instead of reconstructing a parallel result. |
...
| `POL-TYPE-027` | Do not define or use `typing.Protocol` or another structural duck type. Type mathematical values through the exact category-owned `ObjectType`, `ElementType`, or `MorphismType`, and express capabilities through category membership and declared functors. |
...
| `POL-DOC-009` | Declare only mathematically meaningful immediate declared functors. Obtain deeper inherited capabilities by functor composition, not by adding direct functors for convenience. |
...
For example, a lattice specification declares its declared functor to the appropriate formed-module category.
