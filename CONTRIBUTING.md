...
| `POL-CAT-083` | Represent a distinguished named object `X` by a category class, a completely abstract new category. Every category class is a point in `Cat` automatically. Register `X` as a point in `D` by adding the structure functor `D.Point()`, the arrow `* -> D`, to that class; `D.Point()` is the arrow `* -> D` selecting `X`; the kernel carries placement and inheritance along the inclusion of the replete full subcategory it generates, a monomorphism and isofibration (`POL-FUN-036`, D167), and it supplies the codomain's surfaces through the categorical level shift, `D.ObjectType` to `X` itself and `D.ElementType` to `X.ObjectType`, so every object of the category `X` is an element of the set `X` (D128, D154, D161). |
