...
| `POL-FUN-035` | A selected functor retains one pure typed conversion from source construction data to the data consumed by each target constructor. The functor uses this conversion for its public object and morphism actions. The kernel uses the same conversion to initialize target classes on the source instance. Public `F(x)` returns the separate image owned and cached by `F`; inherited methods use direct Python MRO on `x`. For a generalized element `t: T -> X`, public action is `F(t): F(T) -> F(X)`. If `C` and `D` choose separators `G_C` and `G_D`, a supplied morphism `c_F: G_D -> F(G_C)` gives the generalized point `G_D -> F(X)` by composition. Identity and composite functors compose these data by the functor laws. Two selected paths to one target class must produce the same target constructor datum. A conversion reads no partly initialized value. |
