...
| `POL-SCOPE-009` | Judge an architectural claim from the live method owners, named functors, their executable object and morphism actions, the structure-functor class graph, constructor obligations, and universal data. Agent reports, passing tests, route metadata, generated type identities, and runtime output cannot replace that inspection. |
...
| `POL-SCOPE-012` | Kernel code implements category-independent class compilation, private runtime state sharing for selected structure-functor targets, dynamic types, and once-only initialization through Sage's controlled linearization. Each named functor owns its public images through its ordinary actions. The kernel contains no category-specific mathematics and reads no second functor description. |
...
| ID | Policy |
| --- | --- |
| `POL-LEAF-058` | Every leaf functor defines complete executable object and morphism actions. `on_object(X)` returns an object constructed through the target category's public object constructors. `on_morphism(f)` returns a morphism constructed through the exact target hom category. These actions can call any public method of the source category and private helpers owned by the same leaf. They are the only leaf-authored account of the functor. An ordinary functor that is not selected does not participate in class construction. |
...
| `POL-LEAF-061` | A leaf obtains each selected functor in exactly one of two ways. It reuses the exact functor retained by its defining categorical construction, or it constructs the leaf-specific action through `Fun(self, Target)` and supplies its complete object and morphism actions. `Cat` and the kernel never choose that functor or add another description to it. Put a convenience constructor on the applicable functor category or its property subcategory, not in a parallel helper namespace (D119, D120). |
| `POL-LEAF-062` | Keep a method private when its only consumer is a functor action or another leaf-internal implementation. Prefer a local function inside the functor constructor when no other leaf code needs it. Such helpers can inspect the leaf's private state, but they do not enter generated public types, method catalogues, or user completion. A genuinely public mathematical datum remains public even when a functor also uses it (D120). |
...
| `POL-KERNEL-018` | Make each inherited method callable directly on every structural descendant through its compiled class MRO, applying to the descendant with its arguments unchanged. For a selected `F: C -> D`, the ordinary action constructs the separate public image `F(X)`, while the kernel makes the applicable `D` implementation state available on `X`. Thus `X.f() == F(X).f()` holds and the declaring method reads its own state where it is applied. This private runtime obligation uses only the already-complete functor and creates no theory-layer input (`POL-MATH-046`). |
...
| `POL-KERNEL-028` | Set `C.ObjectType`, `C.ElementType`, and `C.MorphismType` from the `parent_class` of the applicable private Sage runtime implementation category. An object of `C` is an instance of `D.ObjectType` for every structure functor `F: C -> D`, and it carries the private data that methods of `D.ObjectType` need. Apply the same compiler rule to the applicable `C.ElementType` and `C.MorphismType` surfaces. |
| `POL-KERNEL-029` | A leaf constructor accepts its own category's data and initializes its own state only. It never accumulates a field for an ancestor category and never calls an ancestor initializer by hand. Each ordinary functor action constructs its separate public image through the target category. When that functor is selected, the kernel makes the target implementation state available on the source instance and initializes every reachable class once without another leaf declaration. Ordering, state sharing, and allocation remain private implementation choices (`POL-DOC-017`). |
...
| `POL-FUN-035` | `F.on_object(X)` is arbitrary ordinary Python that returns an actual object of `F.codomain()` through that category's public constructors. `F.on_morphism(f)` returns an actual target morphism through `Mor(F.codomain())(F(f.domain()), F(f.codomain()))`. These two actions are complete. Selecting `F` changes compiler behavior only. The kernel treats both function bodies as opaque and requires no other writer input (D120). |
