...
| `POL-CAT-071` | Reject a selected structural edge when an inherited target class needs construction input and the declaration supplies no exact typed conversion. Compute conversions before the C3 constructor chain starts. Initialize `Cat().ElementType` from the parent category of each object. Never derive a conversion from adjacent classes in the linear order. |
