...
| `POL-CAT-012` | Construct `C.ObjectType`, `C.ElementType`, and `C.MorphismType` with Sage dynamic classes and Sage's controlled linearization. A shared ancestor class occurs once in the MRO and its initializer runs once. Each named functor keeps its own public images. |
...
| `POL-CAT-044` | Put each exact decision procedure behind the predicate owned by the registered property subcategory. The axiom declaration `P` gives the kernel the functorial construction `C |-> C.P()` and its ambient category. The kernel generates `is_P()` on `C.ObjectType`; the method returns the containment proposition for `C.P()`. Thus `Sets().ObjectType.is_finite()` returns the proposition `X in Sets().Finite()`. A concrete predicate-backed implementation of `Sets().Finite()` inherits `PredicateSubcategory` and implements its private abstract `_predicate()` contract with `X.cardinality() < aleph_0`. Descendants receive the generated method through their compiled classes. `ask()` evaluates the proposition and `__contains__` asks the same one. Exact `True` invokes same-object refinement. A leaf does not write the ambient method, override the predicate with a Boolean method, or apply another category's predicate at a use site. |
...
| `POL-CAT-060` | Model `P` as an axiom that declares the functorial construction `C |-> C.P()` and gives each result its monomorphism into `C`, following Sage's `with_axiom` mechanism. Sage's `_base_category_class_and_axiom = (CClass, "P")` registration connects a concrete category class to that same `C.P()`. The registered axiom identifier supplies the generated application name: convert CamelCase to snake case and prefix `is_`. A class that chooses predicate-backed refinement inherits `PredicateSubcategory` and implements its private abstract `_predicate()` method. The base constructs the containment proposition from ambient membership and that predicate. The kernel generates the ambient application; the concrete category does not name or write it. |
...
| `POL-CAT-066` | Each named functor owns its object and morphism actions. Two functors with the same endpoints can construct different images. A point image uses the functor's morphism action and its declared terminal-object comparison. |
...
| `POL-CAT-071` | Reject a selected structure functor when a contributed target class needs construction input and the functor supplies no exact typed construction rule for an applicable target constructor. The target category can have many constructors. The rule comes from the selected functor, never from whichever classes happen to sit next to each other in the compiled order. Initialize `Cat().ElementType` from the parent category of each object. When the construction rules run is an implementation choice (`POL-DOC-017`). |
...
| `POL-CAT-084` | If `C.P()` is a property subcategory and `D` has a selected subcategory monomorphism into `C`, derive `D.P()` automatically as the corresponding narrowing of `D`. Derive its compiled class, containment proposition, refinement, and inherited construction surface generically. A descendant can add constructors for its own supported representations. |
...
| ID | Policy |
| --- | --- |
| `POL-LEAF-058` | Every leaf functor defines its object and morphism actions. For each functor selected in `structure_functors()`, the leaf also states how one source construction supplies each contributed target class through an applicable target constructor. The target category can have many constructors. The kernel uses the selected functor's construction rule to initialize target state on the structured source instance. An ordinary functor that is not selected does not participate in class construction. |
...
| `POL-LEAF-033` | A concrete category that implements `C.P()` uses Sage's axiom registration. If it chooses predicate-backed refinement, it inherits `PredicateSubcategory` and implements the private abstract `_predicate()` method. It can declare its nested implementation classes and every constructor needed for supported mathematical and engine representations. The kernel owns the monomorphism, generated `is_P()` application, propagation, and same-object refinement. Category placement makes `ask()` return `True`. |
...
| `POL-KERNEL-010` | Keep Sage dynamic-class construction and controlled initializer linearization inside the kernel. Expose the resulting typed mathematical surface through ordinary Python inheritance. |
...
| `POL-FUN-024` | Define fullness, faithfulness, full faithfulness, essential surjectivity, and equivalence as properties of named objects in `Fun(C, D)`. Give each one its property subcategory and owned predicate. Use `Faithful` for injectivity on each fixed-endpoint morphism collection, `Full` for surjectivity, `FullyFaithful` for bijectivity, and `EssentiallySurjective` for object coverage up to isomorphism. State these properties on a structure functor, never on its source category as a substitute for that functor. |
