...
| `POL-CAT-070` | Treat direct implementation construction, private constructors, subcategory monomorphisms, lifts, and internal helpers as category-entry paths. Each path selects the exact established property category through direct construction, an active assumption, exact computation, or construction-owned mathematics. Internal access is not an exemption. Call `_construct`, `ObjectType`, or another raw allocator only inside the owning category constructor after that target category has been established for the exact supplied value. |
...
| `POL-LEAF-059` | Declare a leaf category as a class, which inherits from a base category class and declares `ObjectType`, `ElementType`, and `MorphismType` as its own nested classes. Do not assemble a category by passing externally defined role classes into a call, and never let a kernel index such as the role enumeration appear in a leaf's declaration. |
