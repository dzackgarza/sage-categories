...
| `POL-MATH-034` | Represent every mathematical truth question as an owned predicate. Applying it constructs a proposition. Represent every other mathematical query that is not total and exact on its full declared domain as an owned query with an exact result category. An applied query is a symbolic term, not a proposition. Predicate application, query application, and `ask()` are the complete public query surface. Only `ask()` returns a decision, an owned result, or `Unknown`. |
