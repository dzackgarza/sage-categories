...
| `POL-CAT-099` | Define the dualizing functor `Op: Cat() -> Cat()` with `Op.on_object(C) = C.op()` and `Op.on_morphism(F) = F.op()`. Dualization sends `eta: F => G` to `eta.op(): G.op() => F.op()` and retains the natural isomorphism `Op compose Op ≅ Id`. Derive each dual construction through this operation. Follow [Mathlib, `CategoryTheory.Cat.opFunctor`](https://leanprover-community.github.io/mathlib4_docs/Mathlib/CategoryTheory/Category/Cat/Op.html) and [Mathlib, `CategoryTheory.Opposites`](https://leanprover-community.github.io/mathlib4_docs/Mathlib/CategoryTheory/Opposites.html). |
...
| `POL-KERNEL-003` | A leaf functor states its mathematical action on objects and morphisms. A functor between categories maps their points and generalized elements by composition. Compiled `ElementType` inheritance uses the selected structure functor's typed element-constructor conversion. The leaf never implements path composition or cache management. |
...
| `POL-KERNEL-026` | Compile each inverse-image property subcategory produced under `POL-CAT-084` and `POL-CAT-100`. Build its class MRO, construction dispatcher, containment proposition, refinement, and inherited predicate behavior from the retained category and functors. Do not implement property propagation as a separate compiler operation. |
...
| `POL-FUN-002` | A functor owns its object and morphism actions. For a point `x: * -> X` and a functor `G: X -> Y`, its point image is the composite `G after x: * -> Y`. A selected structure functor contributes target `ElementType` implementation only through an exact typed constructor conversion. |
...
| `POL-FUN-019` | Define `C.Products()` as the full subcategory of `C` on the union of the strict images of the chosen product functors `Prod_J: C^J -> C` for nontrivial discrete shapes `J`; define `C.Coproducts()` dually. Define `C.TensorProducts()` and analogous named constructions by their chosen construction functors. Reach each family through its retained identity-on-values monomorphism. For each input diagram `D`, its constructor returns one value: the constructed object, an object of `C`, placed in the family. The family retains the universal data of each diagram: `D`, its defining morphisms, and its universal maps. Distinct diagrams have distinct universal data, including when the objects they construct are identical. Keep these categories distinct from the full replete subcategory `C.ImagesOfFunctor(F)`, which records only the existential image property. |
...
| `POL-FUN-035` | A structure functor retains one pure typed conversion from source construction data to the data consumed by each applicable target constructor. The functor uses the object and morphism conversions for its public actions. The kernel uses the applicable object, element, or morphism conversion to initialize the target class on the source instance. Public `F(x)` returns the separate image owned by `F`; inherited methods use direct Python MRO on `x`. A conversion reads no partly initialized value. |
