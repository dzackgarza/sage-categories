...
| `POL-MATH-034` | Represent every mathematical truth question as an owned predicate. Applying it constructs a proposition. Represent every other mathematical query that is not total and exact on its full declared domain as an owned query with an exact result category. An applied query is a symbolic term, not a proposition. Forming either application performs no evaluation. Predicate application, query application, and `ask()` are the complete public query surface. Only `ask()` returns a decision, an owned result, or `Unknown`. |
| `POL-MATH-035` | Ask every applied predicate before an assertion or branch. Write `decision = ask(proposition)`, assert that it is not `Unknown` where a decision is required, and then use the decision. The applied proposition's Python truth value raises. |
...
| `POL-ENGINE-004` | Expose a set-theoretic image as `f.image()` and construct a predicate subobject through `Sets().MonoOver(X).from_predicate(predicate)`. Keep constructors such as Sage or SymPy `ImageSet` and `ConditionSet` private. |
...
| `POL-CAT-043` | Let a named property subcategory and its owned predicate state one mathematical condition, declared once as the subcategory's membership proposition. The generated `is_P()` application returns that proposition without evaluation. `ask()` requests a decision and performs exact positive self-refinement. Containment asks the same proposition and forces a Boolean. These are different operations on one proposition, not different notions of membership. Never replace the proposition with an unrelated invariant comparison. |
...
| `POL-CAT-092` | Define the uniform construction methods once on `Cat().ObjectType`; every category inherits them. For `X in C`, use the standard over/under names: `C.MonoOver(X)` is `C.SliceOver(X).Monomorphisms()`, `C.MonoUnder(X)` is `C.CosliceUnder(X).Monomorphisms()`, `C.EpiOver(X)` is `C.SliceOver(X).Epimorphisms()`, and `C.EpiUnder(X)` is `C.CosliceUnder(X).Epimorphisms()`. Here the property subcategory applies to the defining arrow in `C`, through the retained functor from the slice or coslice to `Mor(C)`. The ambient category in the call fixes the role of `X` when the same value belongs to more than one category. Define products, coproducts, pullbacks, pushouts, equalizers, coequalizers, and the other standard universal-construction methods once on `Cat().ObjectType` from `Limits(I)` and `Colimits(I)` at their standard shapes. A leaf supplies only its realization and any constructor specific to its mathematics. `Sets().MonoOver(X).from_predicate(predicate)` is such a set-specific constructor. Restricting a leaf's structure to a subobject is leaf work (`POL-LEAF-060`). Each limit or colimit family is indexed by one supplied shape `I in Cat()`; `C.Limits(Cat())` never denotes limits over all small shapes. A discrete diagram is a rule `i |-> X_i` on an index set; a Python sequence denotes the diagram over `Discrete([n])`. Bicompleteness and cartesian closure of `Cat()` are trusted declarations attached to the constructors the kernel supplies; they assert nothing about a shape or category not supplied to a constructor. |
...
| `POL-CAT-094` | An object of `C.MonoOver(P)` is an object `S` with a monomorphism `j: S -> P`. The equivalence-class formulation of a subobject matters only when deciding whether two such representatives define the same subobject. Let `p` be a selected product presentation with apex `P`, and let `j: S -> P` be such a monomorphism. Make the corresponding object of `Cat().MonoOver(P)` retain `j`. Its component functor at `i` is `p.leg(i) after j`. Do not ask a leaf to repeat these composites. |
...
- Every set can construct `Sets().MonoOver(X).from_predicate(predicate)`.
...
| `POL-LEAF-060` | A leaf states the theorem that its named functor carries a specified subcategory into another specified subcategory. The generic `F.restrict(P, Q)` construction owns the resulting restricted functor and its action on objects and morphisms. For a poset presented as `(X, R)`, `Sets().MonoOver(X).from_predicate(predicate)` constructs the selected set and its monomorphism into `X`. The ordering leaf states that the order restricts to this subobject and constructs the resulting factorization through `PartiallyOrderedSets()`. The generic restriction calculus then supplies the functor between the `MonoOver` categories. A general category contains no poset-specific theorem. |
| `POL-LEAF-061` | A leaf obtains each selected functor in exactly one of two ways. It reuses the exact functor retained by its defining categorical construction, or it constructs the leaf-specific action through `Fun(self, Target)` and supplies its object action, morphism action, and constructor conversions. `Cat` and the kernel never choose that functor or infer its conversions. Put a convenience constructor on the applicable functor category or its property subcategory, not in a parallel helper namespace (D119). |
...
| `POL-LEAF-014` | Ship and maintain design-pseudocode templates for an ordinary structured leaf, a property implementation, a pullback-defined category, a chosen-datum fibration, and a universal-construction realization. Each template contains only the category declaration, minimal default and named constructors, immediate selected structure functors, and sites for new methods or exact handlers. It reuses every functor retained by its defining categorical construction. A template is never imported, executed, type-checked, or held to a spelling criterion. It shows one valid presentation without making other mathematical presentations stale. A template is defective only when it teaches a shape the kernel cannot compile (D118, D119). |
...
| ID | Policy |
| --- | --- |
| `POL-KERNEL-001` | The kernel translates the applicable immediate structure-functor edges into private Sage runtime implementation categories for objects, elements, and morphisms. Sage compiles their dynamic classes. The kernel supplies typed construction inputs and threads each initializer once. Each named functor owns its public images. |
...
| `POL-KERNEL-017` | The kernel gives each private runtime implementation category the applicable immediate structure-functor target categories and the corresponding local `ObjectType`, `ElementType`, or `MorphismType` as `ParentMethods`. Sage `_make_named_class` and controlled C3 determine the reachable MRO. Neither kernel nor theory code enumerates functor paths or compares data assigned to them. Each named functor constructs its own public images. |
...
| `POL-KERNEL-028` | Set `C.ObjectType`, `C.ElementType`, and `C.MorphismType` from the `parent_class` of the applicable private Sage runtime implementation category. An object of `C` is an instance of `D.ObjectType` for every structure functor `F: C -> D`, and it carries the private data that methods of `D.ObjectType` need. Apply the same rule to `C.ElementType` and `C.MorphismType` when the structure functor supplies the required conversions. |
...
| `POL-KERNEL-030` | Cache `_RuntimeImplementationCategory(C, kind)` by the identity of the owned category and the exact implementation-class kind. Normalize categorical level identities before this lookup, including `Mor(C).ObjectType = C.MorphismType`. Anchor each private implementation-class graph on its existing kernel base class. The adapter is private and states no relation in the owned `Cat` graph. |
| `POL-KERNEL-031` | Use Sage `Category._all_super_categories`, `_super_categories_for_classes`, `_make_named_class`, `C3_sorted_merge`, and `dynamic_class` as the implementation-class compiler. Delete repository implementations of class-graph linearization, controlled-base calculation, and dynamic-class identity. Keep the semantic method-collision check because Sage MRO construction does not decide whether two mathematical owners may share one spelling. |
| `POL-KERNEL-032` | Use `CachedRepresentation`, `UniqueRepresentation`, and `cached_method` when every key has ordinary exact equality. Use `MonoDict` and `TripleDict` only when a key contains an owned value whose equality is proposition-valued. Use `dynamic_class(..., cache=True)` when the kernel constructs a class directly. |
| `POL-KERNEL-033` | Bind property implementation classes through Sage `CategoryWithAxiom` and `_base_category_class_and_axiom` inside the private runtime layer. The owned property category, its defining predicate, its inverse images, and its subcategory monomorphism remain in `Cat`. Use `inflection` for the public `is_P` spelling. |
| `POL-KERNEL-034` | Reuse Sage functorial-construction category factories for private family binding, base-category access, method-provider assembly, and exact-key caching. Replace their `super_categories()` deduction with implementation-class edges obtained from owned functors. For products, Sage `CartesianProductsCategory` is a private implementation source; the owned limiting cone retains the diagram, legs, apex, and universal map. |
| `POL-KERNEL-035` | Use Sage `Hom`, `Homset`, `Map`, `Morphism`, and `IdentityMorphism` directly only when the endpoints are Sage parents. Keep generic `Mor` and `Fun` in the owned `Cat` layer, with their parent, domain, codomain, composition, and object and morphism action protocols grounded in the corresponding Sage implementations. Never force an abstract category object to become a Sage `Parent`, and never use Sage `ForgetfulFunctor` for a selected structure functor. |
| `POL-KERNEL-036` | Parse ordinary Python declarations and generated stubs with Python 3.14 `ast`. Use `tree-sitter-sage` only for Sage syntax. Fixed constructor wrappers are ordinary declared functions. If a later generated wrapper needs a runtime signature, construct it with `makefun`; do not add signature-generation code. |
