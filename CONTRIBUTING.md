...
| `POL-CAT-083` | Represent a distinguished named object `X` by a category class whose sole object is `X`. Every category class is a point in `Cat` automatically. Declare a placement of `X` in `D` by adding the structure functor `D.Point()`, the arrow `* -> D`, to that class; it places `X` in `D` and supplies the codomain's surfaces through the categorical level shift, `D.ObjectType` to `X` itself and `D.ElementType` to `X.ObjectType` (D128, D154). |
...
| `POL-LEAF-069` | Red flag: a datum-free constructor, or a one-object category built by hand. A one-object leaf class is a point in `Cat` automatically and places its object by the structure functor `D.Point()` (D128, D135, D154). |
