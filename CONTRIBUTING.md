...
| `POL-CAT-038` | Make the category call `C.P()(...)` the standard public trust boundary for construction into a property subcategory. `C.P()` has exactly the constructors of `C`, through the kernel's functor `C.P() -> C`, and dispatches to the exact constructor for the supplied construction data. Construction in `C.P()` asserts the defining property. A value already constructed is placed by `assume(X.is_P())` (D21, D150). |
