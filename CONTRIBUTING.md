...
| `POL-KERNEL-031` | Review class compilation against [specs/resolution.md](specs/resolution.md): Sage owns category traversal, controlled C3, and dynamic-class construction, while the repository keeps the semantic collision check. Every owned structural diamond compiles with one shared implementation occurrence and nothing more: no diagnostic, no public-image comparison, no coherence data or proof (D37). |
