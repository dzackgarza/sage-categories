...
| `POL-KERNEL-029` | Review selected inherited execution against [specs/resolution.md](specs/resolution.md): a leaf initializes only its own state, each functor action constructs the separate public image, and the kernel supplies inherited target state once. |
...
| `POL-KERNEL-031` | Review class compilation against [specs/resolution.md](specs/resolution.md): Sage owns category traversal, controlled C3, and dynamic-class construction, while the repository keeps only the semantic collision check. |
