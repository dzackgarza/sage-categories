...
| `POL-KERNEL-037` | Every red-flag shape in `POL-LEAF-063` through `POL-LEAF-079` names a missing capability of the framework. The repair adds that capability at its owner — the kernel for threading, retention, refinement and placement, `Cat` for every operation the mathematics defines — and deletes the leaf wiring; a leaf-local patch that keeps the shape is rejected (D133, D173). |
