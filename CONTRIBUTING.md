...
| `POL-CAT-094` | Let `P` be a chosen product and let `j: S -> P` present a subobject. Make the corresponding object of `Cat().Products().Subobjects()` retain `j`, and read `P` as its codomain. Define its `product_projection(i)` as `P.product_projection(i) after j`. Thus every such object has all component functors. Do not ask a leaf to repeat their maps. |
