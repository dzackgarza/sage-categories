...
| `POL-CAT-002` | Use `ObjectType`, `ElementType`, and `MorphismType` for category-owned implementations. The kernel owns `Cat`; every category uses `Cat().ObjectType`, every actual object of a category uses `Cat().ElementType`, every functor uses `Cat().MorphismType`, and every natural transformation uses `Fun.MorphismType`. Thus every `C.ObjectType` inherits `Cat().ElementType` with parent `C`. `C.ElementType` implements actual elements of objects of `C`. `C.MorphismType` is `Mor(C).ObjectType`. `Fun(*, C)`, with terminal category `*`, constructs generalized elements of `C` separately. |
...
| `POL-CAT-066` | Each named functor owns its image cache. Key object and morphism images by the exact source value and mathematical kind. Two functors with the same endpoints keep independent caches. An element image exists only for separately declared element data. |
...
| `POL-KERNEL-003` | A leaf functor states its mathematical action on objects and morphisms. A general functor has no action on actual elements. A structural declaration supplies an element-constructor conversion only when its mathematics defines one. The leaf never implements path composition or cache management. |
...
| `POL-FUN-002` | A functor owns its object and morphism actions. It has no general action on actual elements. `Cat().ElementType` implements actual objects of categories, not functors. Use `Fun(*, C)` when the functor presentation of objects of `C` is required. Treat any element action as separate declared structure. |
...
| `POL-FUN-035` | A selected functor retains one pure typed conversion from source construction data to the data consumed by each target object or morphism constructor. The functor uses the conversion for its public action. The kernel uses the same conversion to initialize the target class on the source instance. Public `F(x)` returns the separate image owned and cached by `F`; inherited methods use direct Python MRO on `x`. An inherited target `ElementType` requires a separate exact element-constructor conversion. Two selected paths to one target class must produce the same target constructor datum. A conversion reads no partly initialized value. |
