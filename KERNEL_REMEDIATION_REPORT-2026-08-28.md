...
> The active execution owner remains `PLAN-pr-8-functorial-kernel-completion`. This report does not change plan status.

## Verdict

The prior report found several real boundary defects.
Its original stale-plan verdict was wrong.
The current revision of that report corrects this point.
...
The equation is semantic.
It does not require a dispatch wrapper that replaces the Python receiver.
...
- Conclusion: this session identifies the time window only.
  It supplies no kernel or leaf requirement.
...
- Its public constructor can start from a plain Python set and an order callable.
...
- The Python receiver remains the structured source instance after the superclass constructors run.

- A functor has an image.
  A target category does not select one functor image.
...
- A leaf must not receive a second implementation-class classification model.
...
The current policy text now states much of this contract.
`POL-CAT-043`, `POL-CAT-044`, `POL-CAT-062`, `POL-CAT-096`, `POL-LEAF-038`, and `POL-KERNEL-018` are the baseline.
The active plan and live implementation still contain the main gaps below.
...
This replaces external `Role` maps and instance-owned implementation-class factories.
...
- `POL-LEAF-059`, which now permits Sage's independent axiom class form;
...
An ordinary functor remains ordinary when a category returns it from `structure_functors()`. Selection changes compiler input only.
...
When selected routes reach one compiled target implementation class, their constructor inputs must be coherent.
The compiler can require identity agreement for that one inherited class.
...
### Functor-owned construction

A selected functor supplies one construction rule for each target implementation class it contributes.
The kernel uses that rule in two call surfaces.
It constructs the explicit public functor image.
It also obtains the exact input for the target superclass constructor.

These call surfaces have different inputs.
They do not create two independent leaf declarations.
The object and morphism conversions also implement the corresponding public functor actions.

For `U: Posets() -> Sets()`, the contracts are different:

- `U.on_object(P)` applies the projection to the owned poset `P = (X, R)` and returns `X`;

- the public poset constructor can accept a plain Python set `X_prime` and an order callable;

- the object construction rule first forms the owned set `X` and supplies the exact input accepted by `Sets().ObjectType`;
...
- `POL-KERNEL-017`, `POL-KERNEL-027`, `POL-KERNEL-029`, and `POL-FUN-035`;
...
The compiler builds the public source implementation class from the selected target implementation classes.
...
This is the later user correction to the earlier receiver-replacement proposal.
The target method runs on the structured source instance after the target state is initialized.
A forwarding descriptor to a separate `F(x)` would violate that correction.

The active plan's D18 decision is partly correct.
...
Use `POL-CAT-062`, `POL-CAT-096`, `POL-LEAF-038`, and `POL-KERNEL-018` as the current baseline.
Then normalize the remaining conflicting sources:

- `POL-CAT-066`, `POL-KERNEL-017`, `POL-KERNEL-028`, `POL-KERNEL-029`, and `POL-FUN-035`;
...
is the proposition owned by `Sets().Finite()`. The mathematical condition belongs in that category once.
Descendants receive the one `Sets()` method through selected structural functors.

`POL-CAT-043` and `POL-CAT-044` now state this one model.
Use them to normalize remaining placement-only clauses in `POL-CAT-060`, `POL-CAT-068`, and dependent specifications.
...
The Boolean boundary must state one result for `Unknown`. The current specifications require a loud failure because Python containment cannot return `Unknown`.
...
The exact property category supplies the first compiled implementation class.
...
`F.on_element(t)` follows from `F.on_morphism(t.defining_morphism())`. A leaf supplies no generalized-element cache.
...
### K1. Finish contract normalization

Use the corrected policy baseline to revise the active plan and remaining conflicting specifications.
...
State the two functor call surfaces and their one owned construction rule.
State compiled inheritance as the consumer of that rule.
...
- Forward-facing sources contain no `structural_image`, `carrier`, or `Role` API.

### K2. Remove `Role` metadata from category declarations
...
Remove `Role`, `role_of`, and role dictionaries from property and construction APIs.

Acceptance:

- An independent `FiniteSets` class is the implementation of `Sets().Finite()`.

- A property category refines the object, element, and morphism implementation classes without a leaf map.

- The compiler derives all three implementation-class families from category declarations.

- Mathematical leaf handlers use category containment or the exact compiled implementation classes.

### K3. Make one functor-owned construction rule serve both call surfaces

Give every functor semantic object and morphism actions.
Give each selected edge one exact construction rule for every applicable implementation class.
Use it for the public action and the superclass constructor input.
Compose these rules in the kernel.
...
- `Posets()(X_prime, order_rule)` accepts a plain Python set and constructs the owned pair `(X, R)`.

- The same functor-owned rule initializes the inherited set state without a kernel construction-input import in the leaf.

- Two ordinary functors with the same endpoints retain their own images.

- Only coherent selected routes can initialize one compiled target implementation class.
...
Use one same-object refinement path for all three implementation-class families.
...
Regenerate static projections only after the runtime public implementation classes are stable.
...
Remove direct calls to the kernel refinement function.
This applies to set properties, set constructions, finite posets, number sets, cardinals, and ordinals.

The leaf can establish a property by theorem or exact computation.
It then constructs through the exact property category.
The property constructor performs the same-object refinement.

Remove `Role` and `role_of` from mathematical handlers.
Use category containment or the exact category-owned implementation class.
Keep the existing equality and membership algorithms at their mathematical owners.

### `Sets()`

Remove calls to `structural_image()` from `sets/objects.py` and `sets/maps.py`. Remove direct `.predicate()` routing from set methods.
...
Store object-specific semantic state in the exact local implementation class initializer.
This includes finite-set enumeration, finite-subset base and size, and power-object base set.
Do not move this state into a universal-family table.
...
Make the object data the pair `(X, R)`. Give the projection functor semantic object and morphism actions.
...
Remove the current `carrier()` API and term.
Use the named projection `(X, R) -> X` when the underlying set is required.
...
Remove the cardinal representative image cache from the leaf.
The exact functor retains its constructed images.
Keep `_select_representative()` as the cardinal-specific algorithm.
...
Store object-specific semantic state in local implementation class initializers.
Store diagram-indexed universal data in the universal-construction family.

## Disposition of the prior report

| Prior finding | Disposition |
| --- | --- |
| V1: the plan is incomplete | Keep open implementation requirements. Treat status fields and dead references as plan maintenance, not kernel defects. |
...
| V10: category methods resolve a route to reach their own state | Keep. This is the primary kernel defect. Constructor chaining removes the lookup and leaf calls. |

## Separate mathematical defect

The cardinality functor remains partial on its declared domain.

`specs/cardinality.md` declares a functor from `core(Sets())` to `Cardinal()`. The same specification permits a set cardinality to be `Unknown`. `src/sage_categories/sets/cardinals.py:987-1001` asserts that every object image is an exact cardinal.
