...
| `monodict-autofix` | **Externally blocked at the central QC owner.** Ruff `SIM401` rewrites membership/indexing to `.get()`, and `PERF102` rewrites `.items()` loops to `.values()` / `.keys()`; both fixes are unsafe for Sage `MonoDict`. `tests/kernel/test_monodict_runtime.sage` pins the actual API and passes under the declared runtime. The commit normalizer explicitly uses `~/ai-review-ci/tool-configs/ruff-global.toml`, so this repository cannot encode the required exclusion locally. **Acceptance:** the central Ruff config marks `SIM401` and `PERF102` unfixable, and the unsafe-fix reproducer leaves the MonoDict forms unchanged. See COMPLAINTS.md, "Ruff unsafe dict autofixes are invalid for Sage MonoDict" | none |
