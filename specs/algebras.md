...
For the presented abelian-group instance, `AbelianModuleTensor(R)` supplies that structure on the exact `Modules(R, SelfAction(AbelianTensor()))` owner and selects it there; its tensor, associator, and unitors use the same retained balancing quotients and descended abelian coherence as the relative bimodule tensor calculus.
