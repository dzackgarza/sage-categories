...
Notation is not mathematics. A magma object has one operation, `operation()`; a monoid object adds `unit_morphism()`; a group object adds `inversion()`. None of them puts an operator on points, and none has an additive or multiplicative property subcategory: notation is not a property, and the neutral category is not split.

The symbols `+` and `*` are renamings of the one generator of the magma theory. A renaming of generators is an isomorphism of presented theories, so its models form a category isomorphic to the neutral one and distinct from it. That category is the product with the one-object category on the symbol, formed with existing `Cat` primitives: `AdditiveMagmas(V) = Magmas(V) × 1_+`, `MultiplicativeMagmas(V) = Magmas(V) × 1_*`, `AdditiveMonoids(V) = Monoids(V) × 1_+`, `MultiplicativeMonoids(V) = Monoids(V) × 1_*`, later `AdditiveGroups(V)`, with `1_s = Discrete({s})`. The first projection is the renaming isomorphism and is retained for access only; names pass only along restriction functors induced by inclusions of presentations, the carrier `(X, μ, s) ↦ X` for the inclusion of the empty theory and `(M, s) ↦ (magma of M, s)` for `{s} ⊂ {s, e}`, and these are the declared faithful isofibrations. Each copy writes exactly its renamed generator: `addition()`, `zero()`, `+`; `multiplication()`, `one()`, `*`. A category built from several operations, `Semirings(C)`, `Rings(C)`, modules and bimodules, is the pullback of the named copies over the carrier; its legs land in distinct categories, so it inherits each name once, the carrier once, and no neutral name. [D167](#d167) and [D37](#d37) are unchanged: the construction never presents the kernel two owners of one name with different data, so no precedence rule is exercised, and issue #26 closes by construction.
...
Source: Claude session `session_016m3JEYoECUqHhZ17hqm45F` (<https://claude.ai/code/session_016m3JEYoECUqHhZ17hqm45F>; local transcript `~/.claude/projects/-home-dzack-gitclones-sage-categories/413f7375-2421-4ebe-a8bb-0180749386a4.jsonl`), 2026-09-05, the design discussion after the order-leaf closure at `414c9f5`. That discussion is the full back-and-forth behind this construction. It rejected, in order: notation as a property subcategory; two functors from one compound category into one structure category, which inherit one attribute name with two data; presentation data attached to functors; two structure categories with their own fields; and a neutral `multiplication()` on monoids, since `(ZZ, +)` is a monoid. It established that a semiring inherits the unit under one neutral name in two inequivalent ways unless the neutral name never reaches it, that `(X, R) ↦ X` is the restriction induced by the inclusion of the empty theory rather than an inclusion of categories, and that the tagged product is the mathematically principled form of the upstream `AdditiveMagmas`/`MultiplicativeMagmas` split. Precedents inspected: nLab magma and monoid objects and magmoidal categories, Mathlib `Mon_ C`, `FirstOrder.Language` with `LHom.reduct` and `Language.ring`, 1lab displayed categories, the HoTT structure identity principle. The user decided the construction and the neutral `operation()`; the choice of `Discrete({s})` for `1_s` and the `point_morphism` primitive are derived implementation decisions.
