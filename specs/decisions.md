...
**D99 (08-29, `77631b59` 2026-08-29T00:44:44Z). `Core` is a functor `Cat -> Groupoids`, and the wide-subcategory construction is unsupported architecture.** Three objects are distinct and were being conflated: a multiplicative property `P` of the morphisms of `C`; the category `W_P(C)` with the objects of `C` and only its `P`-morphisms; and the inclusion functor `i_P: W_P(C) -> C`. A construction produces a category *and* its defining functor, so the category and its inclusion are never two spellings of one thing.
...
`Groupoids` is missing from the repository model, and `Core` is required. The generic public `WideSubcategory(P)` has no independent production use: its only caller is `C.Core()`, and the cardinality functor calls `Core()`. The wide-subcategory requirements in `AGENTS.md`, `README.md`, and `specs/functor.md` are unsupported architecture.

Two defects follow from the same confusion: `cat/wide.py` declares every wide inclusion an isofibration, which holds only when `P` contains all the isomorphisms; and `specs/functor.md` writes `Fun(Wide, C)` with no defined `Wide`, which does not type-check as written.
