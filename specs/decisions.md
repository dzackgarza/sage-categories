...
**D160 (09-02, `54674b9b` 2026-09-02T22:47Z; agent rendering of D156 to D158, recorded for the R0 gate). Spellings the specifications use for D156 to D158.** The identity functor of `x` is `End_Cat(x).one()`, the specification's existing spelling (`POL-CAT-023`), for the owner's "`Fun.Endomorphisms()(1)` or whatever" (D156). The limit-creation property is `is_limit_creating(I)`, the generated `is_P()` of the axiom `CreatesLimits` (D158, `POL-LEAF-068`). A retained composite such as `U = Relations().product_projection(0) * (Posets() -> Relations())` is declared by its composition line, and the theorem on it is stated by `assume(U.is_limit_creating(Discrete))` immediately after, because the leaf constructs no actions for a retained functor (`POL-LEAF-070`) and a constructor is not a placement route (D150); this is the explicit form for a functor the leaf does not construct. A slice's retained projection is `C.SliceOver(X).projection()` by D157's rule through `Op`. `POL-LEAF-080` and `POL-LEAF-081` carry D156 and D159. The templates are `finite-set`, `poset`, `finite-poset`, `pointed-sets`, and `poset-products` (`POL-LEAF-014`, D155).
