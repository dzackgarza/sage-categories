...
**D86 (08-28, `77631b59` 2026-08-28T19:03:51Z). An algebraic identity is named by the operation it is an identity for.** `identity()` unqualified says nothing: `ZZ.identity()` could be `0` or `1`. An algebraic identity comes from the magmatic structure, which splits into the additive and multiplicative axiomatic subcategories, so the names are `additive_identity()` and `multiplicative_identity()`.
...
D84 gives the identity morphism its owner, `End_C(X).one()`; this entry is the other notion. Neither leaves a bare `identity()` on an object.
