...
**D13 (08-26, corrected 09-02, `e38ff397` 2026-09-02T14:40Z; `4544eba5` 2026-08-28T12:18Z, `01a03368` 2026-08-24T23:50Z, `01a04c1c` 2026-08-29T09:15Z; earlier corrections 08-31, `01a04c1c-b22b-7f70-8351-6e12ca028470` 2026-08-29T08:08Z, 2026-08-29T08:42Z, 2026-08-29T09:15Z; `2026-08-30-cce86657` 2026-08-30T18:13Z; `01a05682-23ba-7171-bee9-5755d4c313a4` 2026-08-31T12:04:57Z). The kernel is Sage class-building over a private mirror of the owned implementation graph, plus private runtime state sharing.** Sage's dynamic-class and controlled-C3 machinery works for building the implementation classes. The private Sage runtime categories are generated from the owned `structure_functors()` declarations and are unrelated to Sage's mathematical category graph except for their shared low-level `Parent`/Python ancestry. The compiler makes each applicable target implementation state available on the source instance, initializes each reached implementation class once, and exposes inherited methods as ordinary Python inheritance. A leaf constructor never accumulates a field for every ancestor category. An inherited method runs on the structured source object and reads the initialized target state there; it does not forward through a separate public underlying object. The kernel treats functor bodies as opaque executable actions and uses them: for each selected `F: C -> D`, the kernel runs `F.on_object` on the source value once the source's own local initializer has run, and the datum that action feeds to `D`'s constructor is the datum that initializes the `D` implementation on the same source value. The action uses only the source category's own methods. No second leaf-authored transport declaration exists, and no theory declaration calls a base-class initializer; the kernel runs every reached local initializer itself. Two owner statements govern how the kernel may use an action: on 08-28 (`4544eba5` 12:18Z) the kernel uses the functor definition to thread the constructor arguments into the target's initializer, and on 08-30 (`2026-08-30-cce86657` 01:52 local, 17:52Z) an action may always assume its input is fully initialized. Both hold at once: the kernel runs an action only after the source's own local initializer, and an action uses only the source category's own methods, so every value the action reads is initialized. Diamonds follow D37.

**D123 (08-30, corrected 09-02, `e38ff397` 2026-09-02T14:40Z; `4544eba5` 2026-08-28T12:18Z, `01a03368` 2026-08-24T23:50Z, `01a04c1c` 2026-08-29T09:15Z; earlier corrections 08-30, `01a04e73-53f5-7280-8e1f-48e3a96c204f` 2026-08-30T00:56Z; `2026-08-30-cce86657` 2026-08-30T17:52Z). The two ordinary functor actions are the sole functor declaration.** `F.on_object(X)` constructs and returns the public image in `F.codomain()`. `F.on_morphism(f)` constructs and returns the public image in the exact target hom category. Their inputs are values of the stated source category whose own local state is initialized, and an action uses only that category's own methods. Selecting `F` in `structure_functors()` lets the kernel run the object action, after the source's own local initializer, to initialize the `D` implementation on the source value from the same datum the action feeds to `D`'s constructor (D13). The 08-30 recording `2026-08-30-cce86657` (01:52 local) states that an action may assume a fully initialized input, and the 08-28 session `4544eba5` (12:18Z) states that the kernel threads the constructor through the functor; D13 records how both hold. A leaf supplies only these two actions. Selecting `F` in `structure_functors()` selects the applicable target implementation classes. The kernel treats both actions as opaque and can keep private ephemeral records for Python execution only, but no such record can turn a public functor action into a second constructor-time transport program. Such records carry no mathematical meaning, require no leaf-authored counterpart, and decide neither functor semantics nor category placement. This decision controls D08, D13, D17, D95, D110, D118, and every policy, specification, template, plan, or docstring that can be read as requiring another leaf-authored account of either action.
...
**D65 (08-27, corrected 09-02, `e38ff397` 2026-09-02T16:30Z; `1c1a3599` 2026-08-26T23:08Z, 2026-08-26T23:12Z, 2026-08-26T23:23Z). Cardinals are a semiring with a poset structure.** The poset records that `2 ** aleph_0`, and most cardinal exponentials, are incomparable to `aleph_i` in ZFC. There are two totally ordered infinite sets of formal symbols, `{0, 1, ...}` and `{aleph_0, aleph_1, ...}`, and the algebra must take arbitrary sums, products, and formal exponentials of them.

Do not hand-roll that algebra.
Sage's semiring and poset machinery can compute behind a private engine boundary, but there is no semiring parent in this repository: the owned categories express the operations, `Ordinals()` and `Cardinal()` are categories, and each is a semiring by its point embedding into `Semirings(Cat())` (D126). Declaring an owned value into Sage's own `Semirings()` or `Posets()` puts Sage's mathematical graph into the owned graph (D01). The 08-27 sentence that told the implementation to extend Sage's semiring and poset classes and declare into Sage's categories was not an owner statement; on 08-26 the owner said a semiring parent would be a backend engine and that the categories express the operations.
...
**D110 (08-29, corrected 08-29, `01a04c1c-b22b-7f70-8351-6e12ca028470` 2026-08-29T07:34Z, 2026-08-29T09:15Z). Sage compiles each implementation class.** For each owned category `C` and implementation-class kind `R`, a private runtime category `_RuntimeImplementationCategory(C, R)` has as immediate Sage supercategories the private implementation categories selected by the applicable immediate structure-functor edges. Its one method provider is `C`'s local `ObjectType`, `ElementType`, or `MorphismType`. Sage's `_all_super_categories`, `_super_categories_for_classes`, `_make_named_class`, controlled C3, and `dynamic_class` build the resulting `parent_class`. The repository keeps only the adaptations Sage cannot supply: semantic collision rejection, categorical level identities, and the kernel-run initializer threading of D13; no declaration carries a `super()` call for the kernel to rebind. Corrected 09-02 (`e38ff397` 2026-09-02T17:40Z): the `super()`-rebinding clause came from the 07:34 report in session `01a04c1c`, produced in the owner's ChatGPT session `2026-08-29-52bc359d`; the owner's statements on threading (`4544eba5` 2026-08-28T12:18Z and this session) leave no `super()` in a declaration to rebind. It has no second graph linearizer or dynamic-class identity system.
...
**D131 (09-01, `6660e9c` 2026-09-01; corrected 09-02, `e38ff397` 2026-09-02T17:10Z). CategoryPoint.__eq__ returns AppliedPredicate, not bool.** Under POL-API-015 and `specs/undecidable-properties.md`, mathematical equality on `CategoryPoint` (objects, elements, morphisms) returns `AppliedPredicate` — a SymPy proposition evaluated by `ask()`. This intentionally diverges from Python's `object.__eq__ -> bool` protocol. Mypy's `[override]` diagnostic on `CategoryPoint.__eq__` against `builtins.object` is an acknowledged static consequence of this architectural choice and is not a defect. The candidate parameter of `__eq__` is typed `EqualityInput` and that of `__contains__` is typed `ContainmentInput`, two aliases of `Any` declared once at one site, so the genuinely open input is named and localized (owner, `e38ff397` 2026-09-02T17:35Z; D48). `__bool__` on `AppliedPredicate` raises to prevent silent boolean coercion; three-valued evaluation requires `ask()` (`specs/undecidable-properties.md`).


**D133 (09-02, `e38ff397` 2026-09-02T16:30Z; owner statements `01a03368` 2026-08-24T14:03Z, 2026-08-24T23:10Z, 2026-08-24T23:50Z; `36d46178` 2026-08-26T10:33Z; `1c1a3599` 2026-08-26T23:08Z; `4544eba5` 2026-08-28T12:00Z; `77631b59` 2026-08-28T19:06Z; `b1f26f05` 2026-08-29T04:38Z; `8419143e` 2026-09-01T23:35Z). Engineering in a leaf is a kernel defect, and these shapes are its red flags.** A leaf states its objects, elements, morphisms, constructors from its minimal datum, structure functors, axioms, predicates, and the mathematics first owned there (D77). Everything the structure functors determine arrives from the kernel: implementation inheritance, initializer threading, element construction, identity and composition, object retention, axiom subcategories and their spellings, and placement. A leaf carrying any of the following shapes is evidence that the kernel has not paid for itself, and the repair is a generic kernel capability, never a leaf-local patch: (1) an initializer that calls any base initializer or installs an inherited owner's state by hand; (2) a property or construction subcategory built by hand, named by a string, or patched with an accessor; (3) identity, composition, or morphism construction written for structure the leaf inherits; (4) element construction or retention for elements the underlying structure already supplies; (5) kernel role, placement, or refinement machinery used to branch, or used to refine a value after constructing it instead of constructing into the strongest category; (6) a hand-rolled cache or registry of the leaf's own values; (7) kernel state, such as the constructing category, passed by the leaf into a constructor; (8) a Sage parent, element, or Sage category used as the category's own runtime rather than behind a private engine boundary. The kernel is judged by the absence of these shapes from leaves (Philosophy, "Complexity has a direction"). `POL-LEAF-063` through `POL-LEAF-067` and `POL-KERNEL-037` carry the rule; [`leaves.md`](leaves.md) "Red flags" lists the shapes for a leaf writer.

**D134 (09-02, `e38ff397` 2026-09-02T18:05Z; `2026-08-30-cce86657` 03:09 and 03:11 local). An agent's understanding is not durable; the written record is.** Nothing an agent says in a session persists past it. A decision, correction, or clarification exists only once it is written into its owning document: this file for what was decided, the specifications for the contract, `CONTRIBUTING.md` for the compact rule, and the vault plan for the phase delta and its gate. An agent never reports that it understood or learned something; it reports the artifact that now records it. The authority those documents are checked against is the owner's transcript record in all three stores: Claude sessions under `~/.claude/projects/-home-dzack-gitclones-sage-categories/`, Codex sessions under `~/.codex/sessions/`, and the ChatGPT recordings read through `just -f /home/dzack/gitclones/chat-on-steroids/justfile transcript <id>`. A search that finds nothing is reported as what was searched, what was found, the inference, its confidence, and the gaps; it is never a finding, and no decision is struck or acceptance invalidated on a miss. The record is audited by intent, not by provenance: ask of each mechanism the question a leaf writer would ask, treat an absurd answer as a defect, then confirm the intent in the transcripts. A row's cited session is a pointer to read, not evidence about its content.

**D132 (09-02, `e38ff397` 2026-09-02T13:46Z). Architectural invariants get a mechanical hard stop; conventions do not.** This narrows `D79`. A mechanical check is admissible before 1.0 when it names one acceptance line of [`system.md`](system.md) "Dependency directions" or [`resolution.md`](resolution.md) "Acceptance conditions", or one R-gate criterion, and fails on a file and line. The admitted set, corrected 09-02 (`e38ff397` 2026-09-02T18:30Z) to D133: the layered import contract (`Cat` and the kernel import no production leaf; a leaf imports no kernel internal, without exception); and one rule per D133 red flag, each citing its `POL-LEAF` row: no initializer call of any class inside a theory declaration; no constructor name string, no hand-built property or construction subcategory, and no accessor patched onto a value; no identity, composition, morphism construction, or element construction rewritten in a leaf above the `Sets()` base; no hand-rolled retention of a leaf's own values; no constructing category passed into a constructor; and the Sage boundary as an import contract: only `kernel/sage_runtime.py` and the engine modules the contract names import `sage`, so a Sage parent, element, or category can exist only behind a named engine boundary (D40). The kernel's construction context, which raises when one implementation owner initializes twice, is the model: the check states an invariant the specification already owns, and a breach is a defect at a location. A check on wording, naming, style, or any review judgement stays prohibited under `D79`. The checks run in `just architecture` at the push tier; their rules live in `.ast-grep/architecture/` and `[tool.importlinter]`, and each rule cites its owning line. Grounded in the owner's request in this session for mechanical gate checks that give a hard stop on obvious architecture defects, with a leaf threading base initializers by hand as the named example.
...
**D136 (09-02, `54674b9b` 2026-09-02T19:00Z; `e38ff397` 2026-09-02T14:56Z and 2026-09-02T15:40Z). An R-gate is a record an independent agent writes after the mechanical checks pass; nothing else closes a phase.** The core plan's gates R0 through R6 were recorded as passed at revisions where the criteria were met nominally, by the executing agent, through tests and leaf wiring it wrote itself (the records were commits `f8c335e`, `d721e98`, and `ee17962`, test files the executor wrote and edited until they passed; read with the D132 rules those files carry 43 red-flag shapes, so the witnesses that certified the kernel are evidence of the defects the kernel exists to remove); all seven M-phases were left `in-progress` at once; production leaves and a static-projection track were then built on the unaccepted kernel and reintroduced the shapes D133 names, so that the tree at `c772ccc` fails the D132 checks at 372 locations and the import contract. The owner's instruction: kick the plans back to the early rounds while that violation state stands, and gate every phase on the architecture checks, run by an independent subagent, before any later phase proceeds. The protocol, owned by the core plan card: (1) `just architecture` green at the exact revision is the precondition of every R-gate, and the gate agent runs it first; a red check is a failed gate with no criterion read. (2) The gate agent is a fresh subagent that receives the phase card, the governing specifications, the cited decisions, and the revision, and never the executing session's conversation; the executing agent never grades its own gate. (3) The gate agent's criterion-by-criterion result, at the revision, written on the phase card, is the only acceptance; a commit title, a test file, a report, or a session summary is not one. (4) One phase is active at a time; a later phase leaves `blocked` only when its prerequisite card carries an accepted revision. (5) A generic defect found at any later phase or in any leaf returns every dependent phase to `blocked` and reopens the kernel owner (D93). (6) While an M-phase is open, no production leaf enters the tree (D137), and a red-flag shape in a witness category closes only by a generic kernel capability plus deletion of the witness wiring it replaces, never by adjusting the witness or narrowing a rule (`POL-KERNEL-037`). `POL-DOC-028` carries the rule.

**D137 (09-02, `54674b9b` 2026-09-02T19:00Z; owner's choice recorded in the same session). The pre-acceptance leaves leave the tree.** The five leaf packages (`sets`, `ordinals`, `number_sets`, `posets`, `algebra`) and their tests were written before any R-gate had a valid record; Git keeps them. Each P-phase rewrites its leaf from the templates against the accepted kernel, adds its package to the leaf import contracts, and passes its own gate before the leaf enters `sage_categories.all`. The M-phases are measured on the kernel, `Cat`, and the M5 witnesses; `just architecture` is the push tier's hard stop from R1 on, so the Sage import boundary of D40 (`kernel/sage_runtime.py`) is M1 work, not M6 closure.

**D138 (09-02, `54674b9b` 2026-09-02T19:40Z). A plan anticipates the agent behaviour already observed and carries the gate that stops it.** The owner's instruction: a plan written after witnessing bad agent behaviour must counteract or obviate that behaviour, not describe it. The behaviours observed between 08-30 and 09-02, and the gate that now stops each: the executing agent graded its own gate — the gate agent is a fresh subagent with the card, specifications, decisions, and revision only, and the record it writes on the card is the only acceptance (D136); a test file the executor wrote stood as the acceptance — test files are not evidence, the gate agent writes its own falsifying exercise, and `just architecture` reads `tests/kernel` so witness wiring is found mechanically; every phase was left `in-progress` at once and a phase was marked `complete` with no gate — `just plan-state` at the push tier fails unless exactly one phase is `in-progress`, a `complete` phase carries an accepted revision, and every open or complete phase has complete prerequisites; production leaves and a static-projection track were built beside an unaccepted kernel — the tree holds no leaf before R6 (D137), `just plan-state` fails on a leaf package while the core plan is open, and the leaf import contracts return with P1; leaves were polished to pass checks — a red flag closes only by a kernel capability plus deletion (`POL-KERNEL-037`), never by adjusting the witness or narrowing a rule; decisions were extended with agent clauses — every row cites its session and R0 criterion 7 reads every row against the three stores (D134); an acceptance report was kept out of the repository — the record lives on the card in the vault, whose history shows who wrote it; an agent reported understanding instead of writing — `POL-DOC-024`. `POL-DOC-029` carries the mechanical part.
