...
**D01 (08-22, clarified 08-29). Build the whole foundation before any structured category.** `Cat`, all arrow categories, working inheritance through `ObjectType` and the other implementation classes, and then an extensive `Sets()` that completely replaces Sage's `Sets`. Sage supplies private runtime and computation machinery under D109 through D114; no Sage category or categorical construction enters the owned mathematical graph or public API.
...
**D08 (08-24, corrected 08-29). Every functor has its mathematical actions; selected structure functors also supply construction rules.** Every `F: C -> D` defines `on_object` and `on_morphism`.
Only a functor returned by `C.structure_functors()` contributes target implementation classes to `C.ObjectType`, `C.ElementType`, or `C.MorphismType`.
For each contributed class, the selected functor states how one source constructor call supplies one applicable constructor of the target class.
The target category can have many constructors for different representations.
The selected functor chooses the applicable construction; it does not impose one constructor on the target category.
The kernel uses that same rule to initialize target state on the source instance (`b55dc6aa` 2026-08-27T19:02Z; `4544eba5` 2026-08-28T12:18Z; `01a048f6-e3f5-7e42-be2a-1f60f70ac23e` 2026-08-28T20:21Z).
...
**D34 (08-25, corrected 08-29, `01a048f6-e3f5-7e42-be2a-1f60f70ac23e` 2026-08-28T18:48Z; `01a04c1c-b22b-7f70-8351-6e12ca028470` 2026-08-29T06:02Z, 2026-08-29T06:11Z, 2026-08-29T08:20Z). The `Cat` level owns the construction vocabulary.** It supplies `C.MonoOver(X)`, `C.MonoUnder(X)`, `C.EpiOver(X)`, and `C.EpiUnder(X)` for every `X in C`, together with `C.Products()`, `C.Coproducts()`, `C.Limits(I)`, and `C.Colimits(I)`. These standard over/under names denote the monomorphism and epimorphism subcategories of the slice and coslice categories. `C.Products()` is the interface subcategory on objects in the images of the chosen nontrivial product functors `Prod_J: C^J -> C`; `C.Coproducts()` is dual. The construction retains the diagram and the selected universal presentation. For a product category `P`, an object of `Cat().MonoOver(P)` answers `product_projection(i)`. Slice and coslice categories and their fibrations to the varying object supply the fixed-object constructions.
...
**D103 (08-29, `01a04c1c-b22b-7f70-8351-6e12ca028470` 2026-08-29T07:02Z). M1 supplies a thin functorial calculus in mathematical dependency order.** Its four strata are: functor-category calculus; category constructions; universal and functorial structure; indexed and representational structure. The strata add standard categories, functors, natural transformations, and properties around machinery already required by PR-8. They do not form a second framework beside `Cat`, `Fun`, pullbacks, and the method compiler.
...
**D108 (08-29, `01a04c1c-b22b-7f70-8351-6e12ca028470` 2026-08-29T07:02Z). Fibers, Grothendieck constructions, Yoneda, and representability complete the M1 calculus.** A functor has a category-valued fiber over each object. Base change of a fibration is the ordinary pullback of its projection. The Grothendieck construction owns the passage from an indexed category to its total category. Yoneda, co-Yoneda, restricted Yoneda, and the category of representations are generic constructions. Density and separation are properties of the restricted Yoneda functor. Mates, monads, comonads, Eilenberg--Moore categories, and reflective or coreflective subcategories are later extensions; PR-8 does not implement them.

**D109 (08-29, `01a04c1c-b22b-7f70-8351-6e12ca028470` 2026-08-29T07:34Z). Sage is the private runtime and compiler substrate.** The owned `Cat` layer determines every mathematical category, functor, property, construction, and public type. A private Sage category graph can realize the three implementation-class graphs. No relation in that private graph states a public subcategory relation or changes the owned mathematical graph.

**D110 (08-29, `01a04c1c-b22b-7f70-8351-6e12ca028470` 2026-08-29T07:34Z). Sage compiles each implementation class.** For each owned category `C` and implementation-class kind `R`, a private runtime category `_RuntimeImplementationCategory(C, R)` has as immediate Sage supercategories the private implementation categories selected by the applicable immediate structure-functor edges. Its one method provider is `C`'s local `ObjectType`, `ElementType`, or `MorphismType`. Sage's `_all_super_categories`, `_super_categories_for_classes`, `_make_named_class`, controlled C3, and `dynamic_class` build the resulting `parent_class`. The repository keeps only the adaptations Sage cannot supply: rebinding a copied zero-argument `super()`, semantic collision rejection, categorical level identities, typed constructor conversion, and once-only initializer threading. It has no second graph linearizer or dynamic-class identity system.
...
**D84 (08-29, corrected 08-29, `01a048f6-e3f5-7e42-be2a-1f60f70ac23e` 2026-08-28T18:36Z, 2026-08-28T18:48Z, 2026-08-28T19:09Z; `01a04c1c-b22b-7f70-8351-6e12ca028470` 2026-08-29T08:20Z). Identity morphisms and object-dependent constructions keep their mathematical owners.** For `X in C`, the identity morphism is `End_C(X).one()`, the unit of the endomorphism monoid on `Mor(C)(X, X)` under composition. The inherited fixed-object construction methods are `C.MonoOver(X)`, `C.MonoUnder(X)`, `C.EpiOver(X)`, and `C.EpiUnder(X)`. The ambient category in the call fixes the role of `X`; the same value can occur in more than one category. `Sets().MonoOver(X).from_predicate(predicate)` constructs the set subobject selected by a predicate.
...
**D89 (08-29, corrected 08-29). The kernel generates property applications from axioms.** Declaring the axiom `P` on `C` makes the functorial construction `C |-> C.P()` available and gives each `C.P()` its monomorphism into `C`. A concrete implementation identifies that same axiom through Sage's `_base_category_class_and_axiom = (CClass, "P")` registration. The registered axiom identifier supplies the substitution in `is_P`: the kernel converts its CamelCase spelling to snake case and prefixes `is_`. Thus `"Finite"` gives `is_finite()`, `"FullyFaithful"` gives `is_fully_faithful()`, and `"OfCardinalityExactlyFour"` gives `is_of_cardinality_exactly_four()`. The kernel generates that method on `C.ObjectType`. It returns the containment proposition for `C.P()`. Descendants receive it through compiled inheritance. No leaf writes or separately names the method. `Sets().ObjectType.is_finite()` therefore returns the proposition that its receiver lies in `Sets().Finite()`. Only `ask()` decides that proposition (`4544eba5` 2026-08-28T12:00Z; `01a048f6-e3f5-7e42-be2a-1f60f70ac23e` 2026-08-28T19:51Z, 2026-08-28T22:51Z, 2026-08-28T23:00Z, 2026-08-28T23:05Z).

**D97 (08-28, corrected 08-29). An axiom makes a subcategory available; a predicate can compute refinement into it.** These are two mechanisms and one does not need the other. An axiom in the shape of Sage's `with_axiom` makes `Sets().Finite()` available for trusted construction and refinement. An axiom with no predicate is complete: `ask()` can still use placement, assumptions, and categorical implications. A concrete category registered through Sage's `_base_category_class_and_axiom` mechanism can make that same subcategory predicate-backed. It inherits `PredicateSubcategory` and implements the private abstract `_predicate()` method, here the proposition `X.cardinality() < aleph_0`. The generic base then defines membership in `Sets().Finite()` from ambient membership and that predicate. The registered class is `Sets().Finite()`; it is not a second category (`353b942d` 2026-08-28T14:55Z; `4544eba5` 2026-08-28T12:00Z; `77631b59` 2026-08-28T22:04Z; `01a048f6-e3f5-7e42-be2a-1f60f70ac23e` 2026-08-28T23:00Z, 2026-08-28T23:05Z).
...
There is no bare object to equip when the object is already the pair. An object of `MonoOver(X)` is an object together with a monomorphism into `X` (D74). A construction family that already retains its construction is the equipped form, so it needs no adjective. A product has many isomorphic siblings, and the repository keeps the one it built.

**D74 (08-26, corrected 08-29, `01a048f6-e3f5-7e42-be2a-1f60f70ac23e` 2026-08-28T18:48Z, 2026-08-28T19:25Z; `01a04c1c-b22b-7f70-8351-6e12ca028470` 2026-08-29T06:02Z, 2026-08-29T08:20Z). `MonoOver(X)` retains a monomorphism representative, and restricting structure to one is leaf work.** An object is a pair `(A, i)` with `i: A -> X` monic. The standard category name is `MonoOver(X)`. A subobject in the quotient sense is an isomorphism class of such representatives. Equality of represented subobjects is a proposition.

For a poset `P` presented by `(X, R)`, `Sets().MonoOver(X).from_predicate(predicate)` constructs only a set subobject. `PartiallyOrderedSets().MonoOver(P)` specializes the inherited construction by retaining the restricted order and monomorphism in `PartiallyOrderedSets()`. The ordering subtree decides the strongest established order category of the result.
