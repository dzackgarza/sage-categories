...
**D87 (08-28, `77631b59` 2026-08-28T21:04:34Z). The compiled class inherits the declaration; it need not be the declaration.** The kernel compiles the effective `C.ObjectType` and exposes it under that name. It inherits `C`'s local declaration and inherits `F(C).ObjectType` for each selected functor `F`. Python identity with the nested class is not required, and a new dynamic class is often right, since one declaration serves a parameterized family of categories. A leaf never builds this inheritance.
Two requirements were conflated: that a category writes its local class body as `C.ObjectType`, and that the kernel compiles the effective `C.ObjectType` with dynamic bases from the selected functors. `specs/functor.md` and `POL-API-025` said "those same classes", which reads as identity and conflicts with `POL-KERNEL-028` leaving class construction unspecified. Both are corrected to state the inheritance invariant.
