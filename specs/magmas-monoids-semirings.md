...
`coequalizer_projection(f, g)` is the universal map onto `B / im(f - g)` for a parallel pair of homomorphisms and `coequalizer_mediator(q, k)` factors a homomorphism killing the same subgroup; [bimodules.md](bimodules.md) states the relative tensor product these carry.
