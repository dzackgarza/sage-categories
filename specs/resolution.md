...
| Presented abelian groups, exact homomorphisms, biproducts, coequalizers, and ordinary tensor products | Sage finitely-presented modules; ModulePresentationsForCAP `2026.06-01` | Public objects and arrows remain owned `Ab` values. Coordinate conversion is private boundary data only. CAP owns relation checking, equality of retained presented maps, direct-sum structure, cokernels and their colifts, and finite presented tensor operations; `Sets()` no longer carries a parallel additive map-form engine. |
