...
is a dense test functor.
For every presheaf \(F\), Yoneda gives

\[
\operatorname{Hom}(y(c),F)\cong F(c).
\]

The dense reconstruction presents every presheaf as a colimit of representables.
For module-valued presheaves, free representables give evaluation epimorphisms and cokernel presentations.
[Mathlib implements these generator constructions](https://leanprover-community.github.io/mathlib4_docs/Mathlib/Algebra/Category/ModuleCat/Presheaf/Generator.html).

Sheafification sends presheaf presentations to colimit presentations by sheafified representables.
The Stacks Project constructs sheaves through coequalizers of coproducts of representable sheaves.
See [representable sheaves](https://stacks.math.columbia.edu/tag/00WO).
...
\[
h_X:\operatorname{Schemes}^{op}\longrightarrow\mathbf{Set},
\qquad
h_X(T)=\operatorname{Hom}(T,X).
\]

Thus, \(h_X\) is an object of `Fun(Schemes().op(), Sets())`.

See the [Stacks Project definition](https://stacks.math.columbia.edu/tag/01J5).
For every fiber product \(X\times_ZY\), Yoneda gives
...
Affine test schemes reduce morphism construction to ring maps and descent on overlaps.
Representability returns a pointwise construction to `Schemes()`.

For stacks, the analogous functor is an object of `Fun(Schemes().op(), Groupoids())`.
Atlases and descent diagrams construct objects, isomorphisms, and cocycle data.
This application uses 2-Yoneda and descent in addition to separation and density.
