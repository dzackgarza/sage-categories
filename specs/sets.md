...
Applying it returns a SymPy proposition.
...
Its cardinality query remains unevaluated unless defining data or a construction theorem supplies an exact cardinal.
...
- An unresolved symbolic set contributes no exact cardinal result.
...
| --- | --- | --- |
| Membership | `Contains(x, X)` | Return the category-owned SymPy proposition. |
...
| Set equality or inclusion | symbolic set relations | Return the category-owned SymPy proposition; use `ask()` for its decision. |
