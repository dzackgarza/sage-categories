...
class InserterCategory(LimitSubcategory):
    ...
    def defining_transformation(self) -> NaturalTransformation:
        ...
        return Mor(Fun(self, first.codomain()))(first * self.forgetful(), second * self.forgetful())(lambda value: value.structure())
...
def Inserter(
    ...
    diagonal = pairs.universal_morphism(cone(factors, source, lambda vertex: Fun(source, source).one()))
...
def Magmas(structure: Functor | MonoidalStructuresCategory.ObjectType) -> MagmaCategory:
    ...
    pairs = tensor.domain()
    assert pairs.product_projection(0).codomain() is source and pairs.product_projection(1).codomain() is source
...
def Monoids(
    ...
    def unital(value: CategoryOfCategories.ElementType, left: bool) -> MorphismCategory.ObjectType:
    ...
    equations = (
        (
            transformations(left_unit * forget, forget)(lambda value: unital(value, True)),
            structure.left_unitor().whisker_right(forget),
        ),
        (
            transformations(right_unit * forget, forget)(lambda value: unital(value, False)),
    ...
    for first, second in equations[:-1]:
        inclusion = Fun.full_subcategory_monomorphism(result, pointed) if result is not pointed else Fun(pointed, pointed).one()
...
class AdditiveMonoidsCategory(NamedMonoidsCategory):
    ...
    class ObjectType:
        ...
        def zero(self) -> CategoryOfCategories.ElementType:
            ...
            return _unit_point(self, self._additive_monoid.unit_morphism())
...
class MultiplicativeMonoidsCategory(NamedMonoidsCategory):
    ...
    class ObjectType:
        ...
        def one(self) -> CategoryOfCategories.ElementType:
            """The point the unit selects, when the monoidal unit is terminal."""
            return _unit_point(self, self._multiplicative_monoid.unit_morphism())
...
class AdditiveGroupsCategory(NamedOperationCategory):
    ...
    class ObjectType:
        ...
        def negation(self) -> MorphismCategory.ObjectType:
            ...
            return self._additive_group.inversion()
...
# -- semiring objects: two named monoid structures on one carrier, with distributivity and absorption --


class MonoidPairsCategory(LimitSubcategory):
    """The pullback of ``AdditiveMonoids(C_x).Commutative()`` and ``MultiplicativeMonoids(C_x)`` over ``C``.

    An object is ``(A, M, X)`` with ``A`` an additive commutative monoid structure on
    ``X`` and ``M`` a multiplicative monoid structure on the same ``X``.  Both legs are
    declared faithful isofibrations, so the pair inherits ``zero()`` and ``+`` along one
    and ``one()`` and ``*`` along the other, and the carrier once
    (``specs/magmas-monoids-semirings.md``, "Semirings").
    """
...
class SemiringCategory(EquifierCategory):
    """Semiring objects in a cartesian ``C``: monoid pairs whose distributivity and absorption diagrams commute."""
    ...
    def __init__(
        ...
        self._monoidal, self._pairs = monoidal, pairs
        super().__init__(first, second)
...
def EilenbergMoore(
    ...
    assert multiplication.domain() is endofunctor * endofunctor and multiplication.codomain() is endofunctor
