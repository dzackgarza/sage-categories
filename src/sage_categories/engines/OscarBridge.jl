...
export integer_ring, prime_field, polynomial_ring_with_generators, polynomial_coefficient_map, quotient_ring,
...
"""The canonical map from the exact coefficient ring of a polynomial ring."""
function polynomial_coefficient_map(base, polynomial)
    coefficient_ring(polynomial) === base || error("not the polynomial coefficient ring")
    # OSCAR uses MapFromFunc for coefficient coercions in MPolyMap/MPolyAnyMap.jl.
    MapFromFunc(base, polynomial, value -> polynomial(value))
end
...
function affine_morphism_from_pullback(source_scheme, target_scheme, ring_map)
    morphism(source_scheme, target_scheme, ring_map)
end
...
"""The regular-function pullback on a represented open inverse image."""
affine_open_pullback(map, source_open, target_open) =
    pullback(restrict(map, source_open, target_open))
