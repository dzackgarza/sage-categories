...
and every generating equality decides, and is ``Unknown`` otherwise.  The
projections are the product projections restricted along the inclusion; the
...
2026-08-27).  Its injections are the coproduct injections followed by the
quotient map, and its element equality is an owned predicate whose exact routes
are: two representatives with one index and equal values; for ``I = omega``,
agreement at the larger of the two stages under the transition maps, else
``Unknown``; for a finitely generated shape whose coproduct is finite and
enumerated, the connected components of the generating relation (a union-find
over the finite coproduct, Sage ``DisjointSet``), which decide ``True`` and
``False`` exactly; every other case is ``Unknown``.
...
def limit_of_sets(diagram: Functor) -> SetObject:
    ...
    def compatible(datum: Datum) -> Decision:
        if generators is Unknown:
            return Unknown
        family = product.apex().point(datum)
        return decision_and(
            *(
...
class Quotient:
    ...
    def __init__(self, diagram: Functor, coproduct: ObjectOfCategory) -> None:
    ...
    def partition(self) -> DisjointSet | UnknownClass:
        ...
        if generators is Unknown or not finite.has_chosen_enumeration(self._coproduct.apex()):
            return Unknown
        if self._coproduct not in self._partition:
            components = DisjointSet(list(finite.chosen_enumeration(self._coproduct.apex())))
            for generator in generators:
...
def colimit_of_sets(diagram: Functor) -> SetObject:
    ...
    coproduct_rule = coproduct.apex()._membership_rule
    ...
    if quotient.partition() is not Unknown:
        ...
        apex = sets.ObjectType(sets, membership_rule, Unknown)
    ...
    def injection(member_object: ObjectOfCategory) -> SetMap:
        if member_object not in injections:
            tag = coproduct.coproduct_injection(vertex(member_object))._rule
            injections[member_object] = sets.construct_morphism(diagram.on_object(member_object), apex, lambda value: Representative(quotient, tag(value)))
