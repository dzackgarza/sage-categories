...
is constructed as an object of ``Sets()`` (through ``Sets().Finite()`` when the
product is enumerated and every compatibility decides) and retains its inclusion
into the product, a monomorphism (Mathlib ``Set.inclusion_injective``; inspected
2026-08-26); the projections are the product projections after that inclusion;
the mediating map of a cone is its map into the product.  A subset of a finite
set is finite (Mathlib ``Set.Finite.subset``) and a subset of a countable set is
countable (Mathlib ``Set.Countable.mono``; both inspected 2026-08-26).
...
2026-08-27).  It retains the quotient map from the coproduct, an epimorphism
(Mathlib ``Quot.mk_surjective``; inspected 2026-08-27); its injections are the
coproduct injections followed by the quotient map, and its element equality is
...
__all__ = ["Representative", "colimit_of_sets", "colimit_quotient_map", "limit_inclusion", "limit_of_sets"]

# The inclusion of each chosen limit into its product, and the quotient map of each
# chosen colimit from its coproduct, retained by identity of the apex (POL-FUN-008).
_inclusions: MonoDict = MonoDict()
_quotient_maps: MonoDict = MonoDict()


def limit_inclusion(limit: SetObject) -> SetMap:
    """The retained monomorphism from a chosen limit of sets into the product of its stages."""
    assert limit in _inclusions, f"{limit!r} is not a chosen limit of sets"
    return _inclusions[limit]


def colimit_quotient_map(colimit: SetObject) -> SetMap:
    """The retained epimorphism from the coproduct of its stages onto a chosen colimit of sets."""
    assert colimit in _quotient_maps, f"{colimit!r} is not a chosen colimit of sets"
    return _quotient_maps[colimit]
...
def limit_of_sets(diagram: Functor) -> SetObject:
    ...
    product_rule = product._membership_rule
    ...
    def membership_rule(datum: Datum) -> Decision:
        in_product = product_rule(datum)
        if in_product is False:
            return False
        return decision_and(in_product, compatible(datum))

    decided = tuple((datum, compatible(datum)) for datum in finite.chosen_enumeration(product)) if finite.has_chosen_enumeration(product) else ()
    if decided and all(decision is not Unknown for _, decision in decided):
        apex = finite(tuple(datum for datum, decision in decided if decision is True))
    else:
        apex = sets.rule_valued(membership_rule, Unknown)
        if product in finite:
            finite(apex)
        elif product in sets.Countable():
            sets.Countable()(apex)
    inclusion = sets.morphism_category(1)(apex, product).Monomorphisms()(lambda datum: datum)
    _inclusions[apex] = inclusion
...
def colimit_of_sets(diagram: Functor) -> SetObject:
    ...
    if quotient.partition() is not Unknown:
        apex = finite(quotient.classes())
    else:
        apex = sets.rule_valued(membership_rule, Unknown)
    quotient_map = sets.morphism_category(1)(coproduct, apex).Epimorphisms()(lambda tagged: Representative(quotient, tagged))
    _quotient_maps[apex] = quotient_map
