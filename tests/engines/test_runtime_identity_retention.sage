"""Runtime reconstruction preserves owned identities independently of native handles."""
...
def test_runtime_identity_retention() -> None:
    shared_native_object = object()
    left = Sets(("a", "b"))
    right = Sets((7, 9))
    left_record = retain_finite_native_object(left, shared_native_object, ("a", "b"))
    right_record = retain_finite_native_object(right, shared_native_object, (7, 9))

    assert left_record.native is right_record.native
    assert left_record.value is left
    assert right_record.value is right
    assert left_record.value is not right_record.value
    assert finite_native_object(left).construction.data == ("a", "b")
    assert finite_native_object(right).construction.data == (7, 9)
    ...
    presentation_left = Sets(("x", "y"))
    presentation_right = Sets((1, 2))
    product_presentation = binary_product_data(Sets, presentation_left, presentation_right)
...
test_runtime_identity_retention()
