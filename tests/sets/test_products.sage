...
def test_a_binary_product_is_the_chosen_product_with_projections_that_act() -> None:
    ...
    assert product.apex() in Sets()
    assert product.apex() in Sets().Finite()
    ...
    assert first in Mor(Sets())(product.apex(), two)
...
def test_the_function_set_is_the_exponential_and_the_morphism_category_is_discrete_on_its_points() -> None:
    ...
    assert evaluation.domain() is Sets().Products()((function_set, two)).apex()
...
def test_injectivity_into_a_function_set_is_decided_through_map_equality() -> None:
    ...
    parity = Mor(Sets())(four, two)(lambda datum: datum % int(2))
    collapsing = Sets().transpose(Mor(Sets())((two * four).apex(), two)(lambda pair: pair(int(1)) % int(2)))
    separating = Sets().transpose(Mor(Sets())((two * four).apex(), two)(lambda pair: pair(int(1)) % int(2) if pair(int(0)) == int(0) else int(0)))
